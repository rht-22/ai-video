...
E17-2 의 번인 자막 회피는 *"표본 프레임의 **절반 이상**에서 걸리는 행"* 만 띠로 본다 —
**몇 초씩만 뜨는 방송 텔롭은 못 잡는다.** 이번 편이 증거다(E17 포함 sha `6effc4c` 로
돌았는데 run_log 에 `subtitle_avoid_burned` 단계가 아예 없다 = 미검출). 이건 **KR 전 채널**의
대사·TTS 자막에 걸린 문제이고 E18-4(JP 텔롭 트랙)로는 안 고쳐진다. 고치려면 검출을
**구간별**(텔롭이 뜬 시간창 안에서만)로 바꿔야 해서 회귀 위험이 커 별도 발주로 남겼다.

회귀 가드: `tests/test_e18_style_placement.py`(15건) · `tests/test_e18_localize_telop.py`(11건)
· `tests/test_e15_style_compose.py` 의 제목 회전 절(4건).
