...
- **KO 완성본에도 이미 14.4초 오디오 꼬리가 있었다** — 영상은 25초에서 끝나는데 소리만
  39.4초까지 간다. KR 은 이 검사가 없어 **아무도 몰랐다.** JP 만의 문제가 아니다.
- 그 마당에 0.5초를 이유로 발행을 막는 것은 앞뒤가 맞지 않는다 → 통과시키되 **stdout 에
  '⚠️ 오디오 꼬리' 로 남겨 보이게** 한다(조용히 넘기지 않는다). **꼬리 자체의 원인은
  별건**으로 남아 있다(KR 전 채널 영향이라 신중히 접근해야 한다).
...
- **상류는 멀쩡하다**: `pipeline._resolve_cue_anchors` 가 `end = min(start+duration, total)`
  로 cue 를 영상 안에 가두고, 시작이 `total` 을 넘으면 드롭한다. 이 cue 는 **그 클램프를
  우회했다** — 어디서 샜는지는 아직 못 좁혔다(variant 간 clips/cue 짝 어긋남, 체크포인트
  재개가 후보). 이 안전망은 **상류 원인과 무관하게 증상을 끊는다.**
...
- **범위 실측(2026-08-24)**: `localization_qa` 검수 카드 28건 중 cue 가 영상 밖으로 나간
  편은 **이번 1건뿐**(나머지는 cue 시작이 마지막 자막보다 15~58초 앞). 다만 이 편이
  `style_compose` 를 켜고 돈 **유일한 편**이라 '일회성'과 'style_compose 유발'을 N=1 로는
  구분할 수 없다 — 상류 특정은 계속 남은 과제다.
- 회귀 가드: `tests/test_render_late_cue_guard.py`(11건).
