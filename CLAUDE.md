...
## V3 표본 fps·해상도 계약 (2026-08-31 설정 · 2026-09-01 기록)

⚠ **이 절은 2026-09-01 에 코드를 다시 읽고 쓴 것이다.** 그전까지 CLAUDE.md 의 v3
서술은 8/31 11:19 `dcb4c5d`(사용자 지정) 이전 값(360p·표본 0.5fps)을 그대로 싣고
있었다 — 아래 표가 정본이고, 각 M 절의 실측 수치는 **그 실측 당시 조건**이다.

**파일 fps ≠ 표본 fps.** 파일 fps 는 표본 프레임의 *시각 정밀*을 위한 **상한**이고,
과금·컨텍스트를 먹는 것은 `video_metadata.fps`(표본)다. 파일 fps 를 넘는 표본은
존재하지 않는 프레임을 못 만드니 표본 ≤ 파일이 규약이다.

| 호출 | 파일 | 표본 fps | media_resolution | 상수 |
|------|------|----------|------------------|------|
| Stage 1 전체 훑기 | 480p/10fps | **1.0** | **LOW 명시** | `seq_analyze.SCAN_*` |
| Stage 2 청크 분석 | 480p/10fps | **3.0** | 미지정(기본) | `chunk_analyze.CHUNK_SAMPLE_FPS` |
| M8 정밀 프로브 | 480p/10fps(**무음 `-an`**) | **6.0** | 미지정 | `refine.PROBE_SAMPLE_FPS` |
| Stage 4 스타일 | draft 480p | **6.0** | 미지정 | `stage4.STYLE_SAMPLE_FPS` |

### 토큰 산식과 길이 상한 — **API 실측 (2026-09-01)**

문서가 아니라 `count_tokens`·`generate_content` 로 직접 쟀다(합성 영상, 실소재 미업로드).
`gemini-3.7-flash` · 60초 보정본 6개 fps · 3시간 실물 1건.

```
토큰 = 재생초 × (표본fps × 71 + 32)
```

**오차 0의 선형식이다** — 60초 보정본에서 fps 0.25/0.5/1/2/3/6 여섯 점이 전부 정확히 맞았다
(예: fps=1.0 → 60×71 + 60×32 = 6,180, 실측 6,181). 프레임당 **71**, 오디오 초당 **32**.

- **`media_resolution` 미지정 = LOW 다**(실측 동일: 둘 다 prompt_token_count 5,462).
  즉 **영상에서 해상도를 낮춰 아낄 것이 없다.** `HIGH` 만 약 **3.2배**(17,342) —
  프레임당 71 → 약 280. 구세대 값(기본 258 · LOW 66)으로 적힌 자료를 그대로 쓰면 안 된다.
- ⚠ **`count_tokens` 는 실제보다 약 14% 크게 센다**(60초 6,181 vs 실호출 5,462 · 3시간
  997,381 vs 875,890 — 두 경우 모두 실호출이 ~88%). 그런데 **상한 초과 400 은
  count_tokens 쪽 값 기준으로 난다**(아래 fps=1.0 3시간 사례). ⇒ **계획은 위 산식(보수적)
  으로 세워라.**

#### ⚠ 산식은 둘이고 쓰는 자리가 다르다 (2026-09-01 v4 프로브 · `docs/v4/probes/`)

```
상한 판정(400 초과) : count = 재생초 × (표본fps × 71 + 32)   ← count_tokens · 위 표
과금·예산 집계      : usage = 재생초 × (표본fps × 66 + 25)   ← generateContent usageMetadata
```

- 두 상수가 다른 이유는 가설(`media_processing` 등)이 아니라 **상수 그 자체**다 —
  60초·1fps 로 재면 count 6,180 vs usage 5,460 = 88.3%. 관측된 "실호출이 ~88%"가
  정확히 이 비율이다. HIGH 는 프레임당 264(= 66×4).
- 🛑 **`countTokens` 로 멀티파트(offset) 예산을 잡으면 3.8배 과소 계산된다.** 같은
  파일을 서로 다른 `startOffset`/`endOffset` 으로 여러 번 첨부한 요청에서 영상 토큰이
  **142** 로 나왔는데 실과금은 **546**(= 182×3)이었다. 조각 offset 이 서로 다를 때
  한 파트 분량만 세고(2파트도 3파트도 142), 조각이 *동일*하면 정상 합산된다.
  ⇒ **예산·가드의 정본은 언제나 `generateContent` 의 `usageMetadata`** 다. 재현:
  `docs/v4/probes/mrcheck2.py`.
- `startOffset`/`endOffset` 은 조용히 클램프된다 — 소스 길이를 넘겨도 오류가 없다.
  보내기 전에 길이와 대조하는 것이 계약이다(`clips_beyond_source` 와 같은 규율).

| 표본 fps | 최대 소재 길이(여유 0) | 프롬프트 5만 유보 | 비고 |
|----------|----------------------|------------------|------|
| 0.25 | 5시간 51분 | 5시간 34분 | |
| **0.50** | 4시간 18분 | 4시간 06분 | v3 최초 설계값(스냅 ±2s 와 정합) |
| 0.75 | 3시간 25분 | 3시간 15분 | |
| **0.85** | 3시간 09분 | **3시간 00분** | ★ **3시간이 되는 지점** |
| **1.00** | 2시간 49분 | 2시간 41분 | **현행 Stage 1** |
| 2.00 | 1시간 40분 | 1시간 35분 | |
| **3.00** | 1시간 11분 | 1시간 07분 | **현행 Stage 2**(청크 ≤10분이라 무관) |
| **6.00** | 38분 | 36분 | **현행 refine·Stage 4**(국소 창·draft 라 무관) |
| 24.00 | 10분 | 9분 | API 허용 최대 fps |

프롬프트는 소재 길이에 비례하지 않는다 — 장면 전환 어휘가 `PROMPT_SCENE_CUTS_CAP=1500`
으로 결정적으로 솎이므로(`seq_analyze:94`) 유계다(대략 2~3만 추정, 5만 유보면 넉넉).

### 3시간 실물 실측 (2026-09-01)

합성 3시간(10,800s · 480p · 314MB)을 실제로 업로드해 재고 호출까지 했다:

```
업로드            ACTIVE (364초) — 3시간 자체는 문제없이 받는다
fps 0.85          count 997,381 (95.1%)  → generate_content ✅ 성공(prompt 875,890)
fps 0.90          count 1,035,721 (98.8%) → 상한 안
fps 1.00          count 1,112,401 (106%)  → generate_content ❌ 400 "input token count
                  exceeds the maximum number of tokens allowed 1048576"
```

⇒ **길이 자체의 하드 상한은 없다. 토큰만이 벽이다.**
Vertex 문서의 "Gemini 3.x Flash = 오디오 포함 약 45분" 은 **실측으로 반증됐다**(3시간
통과). ai.google.dev 의 "기본 1시간 / LOW 3시간" 도 구세대 회계로 역산하면 정확히 1M 이
나오는 낡은 수치다(3600×290 · 10800×98) — Gemini 3 에서 기본과 LOW 는 같은 값이다.

⚠ **오디오가 예산의 3분의 1을 먹는다** — 3시간 오디오만 345,600 토큰(상한의 33%).
오디오를 빼면 3시간에 fps 1.37 까지 가능하지만, **빼면 안 된다**(intro/teaser 판정의
음악·톤 단서 — `build_scan_proxy` 독스트링). 오디오는 fps 와 무관하게 32/s 고정이라
**fps 를 낮춰도 이 몫은 안 줄어든다.**

### 표본 fps 자동 결정 (2026-09-01 구현)

`seq_analyze.resolve_scan_fps` — **소재 길이가 Stage 1 표본 fps 를 정한다.** 순수·결정적
함수이고 배선은 두 곳(파이프라인 사전검사 · `run_seq_analyze` 정본)이다.

- **회귀 0 이 조건이다** — `SCAN_SAMPLE_FPS`(1.0)로 예산에 들면 그대로 쓴다.
  **161분까지는 종전과 한 프레임도 다르지 않다**(지금 도는 소재는 전부 여기다).
  162분부터만 낮춘다. 경계·계단은 테스트가 값으로 고정한다.
- 낮출 때는 `SCAN_FPS_QUANTUM`(0.05) 계단으로 **내림** — 같은 소재는 늘 같은 fps 를
  받는다(결정성 합격 조항). 3시간 → **0.85**(실호출로 성공을 확인한 그 값).
- **하한은 임의값이 아니라 스냅 관용에서 파생된다**:
  `SCAN_SAMPLE_FPS_MIN = 1.0 / SNAP_TOLERANCE_SEC` = 0.5. 표본 간격(1/fps)이 관용
  (±2.0s)을 넘으면 모델이 관용 안으로 경계를 제안할 근거를 화면에서 못 본다 — 그 아래로
  내리면 Stage 1 은 어차피 스냅 반려로 재질의를 소진하고 죽는다.
- 하한으로도 안 들어가는 소재(**246분까지 통과 · 247분부터**)는 **크게 실패한다** — 메시지에 필요 fps ·
  하한에서의 최대 길이 · 하한의 유래를 함께 싣는다(조용히 더 내리면 Pro 호출 3회를
  태우고 죽는다).
- **사전검사가 프록시 인코딩 앞에 있다**(`pipeline.py` — `resolve_scan_fps` 가
  `build_scan_proxy` 보다 먼저 온다. 테스트가 이 순서를 문자열 위치로 고정한다).
  3시간이면 인코딩 수 분 + 업로드 실측 364초라, 비싼 단계 뒤에 검사를 두지 않는다는
  이 레포의 교훈(P4 실런)을 따른 배치다.
  ⚠ 다만 **Stage 1 이 실제로 돌 때만** 건다(조건이 Stage 1 분기와 같다) — `stage1.json`
  캐시를 재사용하는 재개(`--from-step render` 등)를 이 가드가 새로 죽이면 안 된다.
- 길이를 모르면(≤0) **판정하지 않고 기본값을 쓴다**(오판 금지 — 안전장치가 멀쩡한 편을
  막으면 안 된다).
- 기록: run_log `steps[{step:"proxy",kind:"scan"}].sample_fps`(사전검사) ·
  `steps[{step:"seq_analyze"}].sample_fps`·`sample_fps_note`(실제 쓴 값·사유·예상 토큰).
  ⚠ 이 기록이 생기기 전까지는 "이 편이 몇 fps 로 분석됐나"를 되짚을 수 없었다.
- ⚠ **Stage 2·refine·Stage 4 는 그대로다.** 각각 청크(≤10분)·국소 창(180s)·draft(~1분)
  라 길이 상한과 무관하다 — 자동 결정을 그쪽에 옮길 이유가 없다.
- 회귀 가드: `tests/test_v3_scan_fps.py`(37건 — 회귀 0 경계·실측 대조·단조·결정성·
  하한 유래·fail-loud·배선 순서).

### 이 값들에 근거가 없다 (2026-09-01 조사)

- 네 상수 중 **기술적 사유가 적힌 것은 Stage 4 하나뿐**이다(스틸 16장 → draft 영상
  6fps: "컷 리듬 기반 팝인·효과음 큐 판단은 정지 프레임으론 불가"). 커밋 `51957ca`
  에 전후 실측이 있다 — 스틸일 때 산출이 프리셋과 **diff 0**, 영상으로 바꾸니 비트별
  차등 연출. 나머지 셋의 주석은 "2026-08-31 사용자 설정"이 전부다.
- **fps 를 올려서 좋아졌다는 실측이 없다.** CLAUDE.md 의 v3 실측(M1·M2·M7·M8)은 전부
  `dcb4c5d` **이전** 커밋의 산물이다 — "M8 7편 fail 0" 도 표본 기본 1fps 의 수치이고
  현재의 6.0fps 프로브를 정당화하지 않는다.
- **회귀 가드가 없다.** `tests/` 에 표본 fps 참조 0건(파일 fps `== 10` 단언만).
  값을 되돌려도 CI 가 못 잡는다.
- **run_log 에 남는 것은 Stage 4 뿐**(`sample_fps`). Stage 1 은 파일 쪽만, Stage 2·
  refine 은 표본 fps 를 아예 안 남긴다 — 산출물에서 "이 편이 몇 fps 로 분석됐나"를
  되짚을 수 없다. **fps A/B 를 하려면 이 기록이 선행돼야 한다.**
- 삭제된 옛 근거(참고): `dcb4c5d` 이전 주석에는 사유가 있었다 — 표본 0.5fps 는
  "**해상 한도 ±2s 와 정합**"(= `SNAP_TOLERANCE_SEC` 2.0 과 맞춘 값), 360p 는
  "시각 정밀도는 스냅이 보정". 값을 올린 커밋이 그 문장들을 지웠다.
...
- **Stage 1**: scan 프록시(**480p/10fps 파일**)를 Files API 업로드,
  **`video_metadata.fps=1.0`** + `media_resolution=LOW` + **temperature=0.0**(결정성
  합격 조항 — GeminiConfig 에 온도 필드가 없는 3.x 규약의 의도적 예외, 이 호출
  한정) + Pro 슬롯(영상을 실제로 보는 호출 규칙). 67분 ≈ **41만** 토큰 — 길이 상한은
  §V3 표본 fps·해상도 계약 참조(2시간+ 소재는 발주서 멈춤 시점 2).
  ⚠ 아래 스모크 실측은 **표본 0.5fps·360p 시절**(6e57e63)의 수치다 — 8/31 11:19
  dcb4c5d 가 값을 올린 뒤 같은 소재를 다시 잰 기록은 없다. chunk: ≤10분 sequence 는 코드가 1개 자동 채움,
...
- **chunk_split**: Stage 1 경계대로 **원본에서 직접** 480p/10fps 재단.
  ...
  시각 환산 정본.
- **Stage 2 표본**: `video_metadata.fps=3.0`(`CHUNK_SAMPLE_FPS` · 2026-08-31 dcb4c5d
  가 종전 기본 1fps 에서 상향). `media_resolution` 은 **미지정**(모델 기본).
  아래 실측(c5670c0)은 그 상향 **이전** 조건이다.
...
- **가왕쇼 잔존 원인은 프롬프트가 아니라 입력 신호**(스캔 프록시 실물 진단):
  ...
  (⚠ **그때의 Stage 1 조건**이다 — 현재는 표본 1.0fps·480p).
