...
## V3 사람 편집 흐름 체인 — `--story-flow human` (2026-09-03, 사용자 결정 · 전면 개편 1차)

`app/v3/story_flow/`(common·select·narration·cover·__init__) · pipeline `_run_m3` 분기 ·
assemble `cover` 클립 · CLI `--story-flow {legacy|human}`. **미지정(legacy) = 종전
`story.run_story` 그대로**(지문에 키를 안 넣어 기존 잡 캐시도 그대로 — 회귀 0).

배경(실측): 종전 단일 프롬프트는 79k자(span 1,373행 + 규칙 11개·심사표·자기점검)라
모델의 최적해가 1초짜리 리액션 파편 나열이었고, 내레이션은 1.03s 창에 "fit" 배치 →
Flash 축약 → "싸우다 불붙었죠"로 나갔다(26자 문장이 7자로). 루브릭 8항목엔 '대사가
이어지는가'가 없었다. 사람이 하는 순서대로 쪼갠다:

```
1 topic(주제)  → 2 scenes(씬 = meaning 단위, 배경/맥락/과정/결과/반응 + 제목)
→ 3 lines(고른 씬의 span 만 재료 — 100행대) → 4 narration(문장 확정 → very_fast 즉시
합성 = 길이 L 정본) → 5 cover(L 에 맞춰 그 자리를 **다시 본다**)
```

- **걸음 1~4 는 Flash 텍스트**(모델 정책 그대로), 걸음마다 반려·재질의 ≤2회
  (`_loop`) · 소진 시 **크게 실패**(폴백 편성 없음 — 얇은 안이 조용히 채택되던 실사고).
  3안 경쟁·8항목 루브릭은 이 체인에 없다 — 품질 게이트는 초안을 실제로 보는 watch_trim.
- **걸음 3 은 낱개 대사가 아니라 구간이다**(2026-09-03 사용자 결정 — 반려가 아니라
  **형식**으로 막는다): 비트 = `first`~`last` + `skip`. 사이 조각은 전부 들어가고, skip
  으로 뺀 구멍이 `SKIP_MAX_VOICED_SEC`(3s)·`SKIP_MAX_LINES`(2줄)을 넘으면 코드가 그
  자리를 **비트 경계로 승격**(`split_at_holes`)하고 `compute_jumps` 가 점프(구멍 승격 +
  5초 초과 간격)를 기록한다. 낱개 id 형식은 모델이 텍스트만 보고 "그럴듯하게 붙는 두
  줄"을 골랐다(실사고: 「손을 좀 비워가지고」→ 23초·6줄 건너뛴 「서로서로 도와야죠」).
  짧은 추임새 솎기는 skip 으로 남고, 초안을 본 뒤의 호흡 컷은 watch_trim 몫.
- **걸음 4 는 화면 id 를 함께 낸다**: 내레이션마다 `cover`(고른 씬의 대사로 안 쓴 조각
  id — `available_covers`)를 지정한다. 기록에 없는 화면은 지정할 수 없으므로 "화면에
  없는 말"이 형식상 막힌다(실사고: 발장난 문장 밑에 깨진 잔 수습 장면 — 프로브 창을
  앵커 쪽 12초로 잘라 발장난 화면이 보이지 않았던 것도 원인, 창 자르기 폐지). 점프
  자리는 편성표에 ⚠ 로 박히고 `required` 빈칸이라 다리가 빠질 수 없다. 내레이션은 절대
  줄지 않는다 — 한 문장 공백 제외 16자 권장(레퍼런스 3편 실측 10~14자·2.0~2.7s), 넘으면
  문장부호·쉼표에서 나누고 24자 넘으면 반려. 합성은 `ko_female`·`fast`(ElevenLabs 1.1 —
  very_fast 실측 7.4자/초가 참고 5~6자/초보다 빨라 한 단계 내림) 한 번. cue 에
  `audio_path`·`measured_sec` 가 실려 resources 가 **재합성 없이 복사**한다
  (`synthesize_tts_with_fit`·shorten 경로는 legacy 전용).
- **덮개(cover) 규칙** — 참고 쇼츠 3편(work/nar_ref, LLM 제작) 실측: 내레이션 밑 화면은
  **내레이션이 서술하는 그 장면**이고 발화 클로즈업이라도 그대로 뮤트해 덮는다(화면
  종류·importance 판정 없음). 후보 순서: ⓪ 걸음 4 가 지정한 화면(designated — L 에 못
  미치면 인접 미사용 조각으로 넓힘) → ① 앵커 앞 빈 구간(lead_in) → ② 같은 씬 미사용
  구간(B-roll) → ③ 고른 씬 전체의 미사용 구간 → ④ 앞 여백 + 대사 머리 일부 뮤트
  (lead_spill) → ⑤ 대사 머리 뮤트(mute_head, 참고 쇼츠 방식 · ⚠ 로그+note). 엔딩(after)
  덮개는 내레이션 끝에서 뚝(꼬리 여유 없음). 대사로 고른
  조각과 이미 놓인 덮개는 ①~③에서 재생하지 않는다(같은 화면 두 번 금지). B-roll 은
  **고른 씬 안에서만**(편 전체에서 끌면 다른 줄거리가 섞인다).
- **프로브** — 창이 L 보다 1초 이상 넉넉하면 그 창(≤30s **통째로** — 넘으면 지정 화면
  중심, 480p/10fps)을 잘라 Flash 에
  시작점을 묻는다(`refine._cut_probe_clip`·`_call_probe` 재사용, 편당 `FLASH_BUDGET` 10).
  답은 scene_cut → grid 경계 순으로 0.3s 스냅. 실패·예산 소진 = 앵커에 붙이는 산술 배치
  (기록). 프로브 클립은 `<job>/cover_probes/`, 합성본은 `<job>/narration/`.
- **경계면**: 산출은 `checkpoint_story` 계약 그대로(template `human_flow`) + additive
  `beats[].covers`. assemble 이 덮개를 `cover` 클립(원음 끔)으로 내고 `verify_edit_plan`
  이 `cover` 키를 tail_pad·head_trimmed 와 같은 지위로 인정한다(출처가 합성 실측 +
  국소 재관찰 — 시각 환각 방어 위반이 아님). `pad_voiced_tails` 는 cover 클립을 안
  건드린다. 덮개 끝이 span 중간이면 그 비트 `head_trim_sec` 로 시작을 당긴다.
- 예산 초과는 산술로 안 자른다 — `budget.unmet` 로 watch_trim 예산 컷 몫(종전 규약).
- 곁다리(같은 날): `assemble.TAIL_SIL_PAD_SEC` 0.05→0.2(의문형 어미 꼬리 잘림 —
  "아, 그런가?" 실사고) · renderer 가 효과음 입력에 `atrim=end=영상길이`(6.6s whoosh 가
...
### 신병4 EP1 첫 실런 후속 3건 (2026-09-04)

- **`--retry-failed-chunks`** (`pipeline.drop_failed_chunks`): Stage 2 캐시에서 실패 기록
  (meanings 없음)만 빼서 그 청크만 재분석 — 성공 청크 재과금 없음. `--from-step chunk_analyze`
  는 전부 재실행이라 s1c0 하나(출력 절단으로 3회 소진) 때문에 Pro 6회를 낭비하던 것.
  stage2 가 바뀌면 story 지문이 갈려 story 부터 자연히 재구성된다.
- **마지막 음절 규칙 완화**(`textcheck.JOSA_EOMI_SYLLABLES`): 마지막 음절만 다를 때 초성만
  허용하던 것을 **어느 쪽이든 어미·조사 음절일 때만** 그렇게 하고, 둘 다 아니면 명사 오인식으로
  보고 자모 ≤2 를 전부 허용한다(행정법→행정반 · 강태환→강태한 · 흥신수→흥신소 · 미쉘→미셸).
  드라이런 95 → 99 적중, 신규 오작동은 시간 지시어·감탄사(이제→이따 · 아유→아주)뿐이라
  `ALIGNED_STOPWORDS` 에 넣어 막았다.
...
- **제목 이중 축소 제거**(`RenderInputs.title_prefit`, 2026-09-04 "제목 좀 더 커도"): v1 렌더러는
  ① `_max_chars_for`(960÷크기, 공백도 1자)로 재줄바꿈해 줄 수가 3·4면 0.85/0.70 축소, ②
  `_scale_font_for_length` 글자수 표(14자 0.90 … 16자 0.77 … 20자 0.60)로 줄별 축소 — **이것이
  사용자가 기억한 "제목 폰트 기준 글자수 조정"이다**. v3 는 `fit_title_sizes` 가 폭을 실측으로
  이미 맞추므로 둘이 겹치면 이중 축소(신병4: 74/85 → 57/76, 폭의 69%만 사용). v3 는
  `title_prefit=True` 로 ①②를 건너뛴다(None/False = v1 바이트 동일). 결과 74/85 = 폭 980 꽉 참.
...
- 비트 검출은 4s 창·2s hop 이라 가장자리에서 ±2~4s, 댄스 브레이크·화음 구간에서 수 초씩 빈다.
  듀엣 무대(m033~m034)가 [2008~2036][2042~2060][2084~2120][2130~2144] 로 끊겨 틈의 가사 3줄이
  자막으로 새고, 첫 구호('가자!' 2006.7)도 창 밖이었다.
- Stage 2 사건 단위가 노래 근거(`SING_HINT`)를 가지면 **그 단위 안의 확정 창을 하나로 잇고**,
  단위 경계가 창에서 `EDGE_EXTEND_MAX_SEC`(10s) 안이면 경계까지 늘린다. **늘리기만** 한다 —
  다음 단위로 넘어간 창을 자르면 전유진 편 2216~2398 이 2305 로 잘렸다(첫 판 결함, 테스트 고정).
  경계가 멀면 안 늘린다 — MC 멘트("여러분 기다리셨습니다" 2199~2213, 창 2216)가 산다.
- 드라이런(가왕쇼 7화): 확정 30창 → 11창. 회귀 가드: `tests/test_v3_singing_skip.py`(+1).

### 재렌더 결정성 3건 — 리서치 예산 · watch_trim 컷 재적용 · overrides 강제 완화 (2026-09-08, 가왕쇼 네 번째 편)

- **Stage 2 리서치 예산 1,200 → 3,000자**(`chunk_analyze.STAGE2_RESEARCH_MAX_CHARS`): 1,200자는 인물
  목록 중간에서 잘려 `[핵심 갈등/서사]`(누가 누구와 겨루는지)가 프롬프트에 없었다 — 참모(최수호)와
  가왕(박서진)의 허세 대화를 둘의 신경전으로 오독한 원인. 프롬프트 문구는 그대로, 잘라먹던 것만 안 자른다.
  ⚠ 청크 캐시 지문(plan+spans)에는 안 들어가므로 기존 잡은 그대로(재분석 시에만 반영).
- **watch_trim 컷 재적용**(pipeline `_run_m4` 캐시 3): M3 가 매 실행 story 에서 조립하므로 M4 에 오는
  지문은 늘 트림 **전** 값이다. pre 지문 일치 + 컷 있음 = 같은 초안 → 캐시된 컷을 그대로 재적용한다
  (`audit.replayed`). 종전엔 이 경우가 빠져 재렌더마다 Flash 를 다시 물었고(1.44s vs 3.44s 실측),
  post 지문·style 지문이 갈려 **사람이 승인한 라벨이 다음 렌더에서 증발**했다.
- **`--edit-overrides` 의 `from_step=draft_render` 강제는 clips 를 바꿨을 때만**: 자막·제목·cue 만 고친
  오버라이드에도 강제해 초안·watch_trim 캐시가 전부 무시되던 것. 그 외는 지문이 판단한다.
- 사람 승인 라벨은 `checkpoint_style.json` `style.v3_style.labels` 에 직접 넣는다(style 캐시가 유지되는
  한 살아남는다 — 위 두 수정이 그 조건). 실측(ep7ex03 r8): draft 재렌더 → 컷 1개 재적용 → style 캐시 로드
  → 라벨 유지.

### 라벨 얼굴 회피 `app/v3/label_faces.py` (2026-09-08, 사용자 지적 · 가왕쇼 네 번째 편)

- Stage 4 는 6fps 저해상 초안을 보고 라벨 `x·y` 를 적을 뿐이고 프로브는 표정·타이밍만 본다 — 가림은
  아무도 검사하지 않았다(v1 E19-4 는 리프레임 얼굴 박스를 쓰는데 v3 에는 그 박스가 없고 가왕쇼는
  얼굴 추적도 꺼져 있다). 라벨 두 개가 얼굴 위에 앉았다.
- 렌더 직전(`finalize` 라벨 ASS 조립 앞) 라벨 창의 초안 프레임(시작 직후·중간)에서 얼굴을 검출해
  캔버스 좌표로 환산하고(밴드 = 소스를 높이 맞춤으로 키운 뒤 가로 중앙 크롭 — `draft_box_to_canvas`),
  라벨 박스(`stage4.label_x_range` 와 같은 폭 자)가 겹치면 **위 → 얼굴 반대쪽 옆 → 같은 쪽 옆 → 아래**
  첫 성립 자리로 옮긴다. 성립 = 모든 얼굴과 14px 이상 떨어짐 + 밴드 안 + 캔버스 가로 안. 안 되면
  **옮기지 않고 기록**. 결정적이라 체크포인트에 안 남긴다.
- 검출기: 번들 YuNet ONNX(`app/assets/models/`)가 있으면 YuNet(옆얼굴·작은 얼굴에 강함 — 10.5s 프레임
  4/4), 없으면 OpenCV Haar 정면 캐스케이드(3/4). 둘 다 OpenCV 내장 API 라 의존 추가 없음. cv2·초안
  부재·검출 실패 = 무회피(안전장치가 연출을 막지 않는다).
...
### 8화 r3 검수 후속 — 되감기 표지 · 메아리 조각 · 놓친 말 복원 (2026-09-09 저녁, 사용자 지적 2건)

- **되감기 표지**(`select.compute_jumps` `rewind` 키 · `narration.REWIND_MARKERS`·`has_rewind_marker` ·
  `validate_narrations(rewind=)` · PROMPT 규칙 한 줄 · `beats_block` "⚠ 되감기"): 결과를 먼저 보여준 뒤 원본 앞으로
  돌아가는 점프의 다리가 「신포시장으로 향하던 중,」이라 시청자에게 그것이 방금 본 결과의 **이전** 과정임이 안 닿았다.
  compute_jumps 가 되감기(gap<0)를 표시하고 걸음 4 검증이 그 다리 문장에 이전 시점 표지(사실·시작은·몇 시간 전·
  이야기는·거슬러…)를 요구한다(형식 — 텍스트 지시만으로는 장소 전환 문장으로 되돌아온다). rewind 미지정 = 종전.
- **메아리 조각**(`assemble.is_echo_fragment` · kind `echo`): whisper 가 「저희 300장 들고 왔어요」 직후 확신 0.10 짜리
  「들고」를 따로 span 으로 냈고 화자 색까지 달라 노란 「들고」가 혼자 떴다. 단어 1개 · prob<`ECHO_MAX_PROB` 0.2 · 직전
  유성 span 의 **어절과 완전 일치**(부분 문자열이면 「어?」가 아무 데나 붙는다) · 소스 틈 ≤1s 이면 버린다. 직전 span 은
  **클립을 넘어 유지**(편성이 같은 대화를 클립 둘로 재단해 「들고」는 다른 클립이었다 — 판정은 소스 시각 틈이라 점프
  뒤엔 안 걸린다). 감탄사(「와!」)는 앞 줄에 없으니 산다(E14 규율).
- **놓친 말 복원**(span 폴백 `scene_backed_heard` 단어 1개 허용 + `word_subtitles` 이웃 화면 `_neighbor_scene` + 창 확장
  `_silent_lead`): whisper 가 「저희 300장 들고」를 놓쳐 그 0.86s 를 무성으로 잡고 「왔잖아요?」만 남겼다. 청취
  「저희 300장 들고 왔잖아요」의 어간(300장)은 **앞 무성 조각**의 화면 묘사에 있다 → 같은 사건 단위의 앞뒤 조각 화면
  묘사도 증거로 보고, 청취가 whisper 보다 길고 바로 앞이 무성 조각이면(≤3s) 그 조각 시작부터 줄을 편다.
  드라이런(8화): 단어 1개 폴백 10건(용하시네요·파이팅·긴가민가·촉이 와요·…) 전부 타당 · 메아리 2건(들고·어?).
- 회귀 가드: `tests/test_v3_plan.py`(+1) · `tests/test_v3_heard_priority.py`(+2). 전체 2199 통과.
- **렌더 캐시 지문 확장**(같은 날 실사고): 위 자막 규칙만 바꾼 `--from-step resources` 재실행이 "지문 일치"로 옛
  최종본을 그대로 내보냈다 — 렌더 지문(`_fp_parts`)이 타임라인·라벨·스타일·design 만 봤다. 이제 자막 세그먼트
  (start/end/text/speaker/color/style)와 cue(문구·start·합성 파일 sha1)가 지문에 든다. ⚠ 기존 잡은 다음 재개에서
  한 번 다시 렌더된다(10초). 회귀 가드 `tests/test_v3_gap_extras.py::test_render_fingerprint_covers_subtitles_and_cues`.

### 8화 두 번째 편(ep8ex01) 첫 렌더 검수 후속 3건 (2026-09-09 밤)

- **`SING_HINT` 에 라이브·버스킹**: 「즉석 라이브를 펼치며」로 적힌 노래 단위가 근거 없음으로 기각돼 가사 2줄
  (「첫눈에 반해버린 사람아」)이 자막으로 샜다. 첫 편 잡에서는 같은 단위가 확정됐었는데 s3c0 재분석 뒤 단위 번호가
  밀리며 겹침 판정이 달라진 것 — 근거 어휘가 좁았던 게 원인.
- **`span_sings` 발화 예외**(`SPEECH_HINT`): 조각 화면 묘사가 발화(알린다·말한다·묻는다·고지…)를 적고 노래 말이 없으면
  단위 문장이 노래(라이브)라도 그 줄은 산다 — 무대 단위 안의 제작진 고지 「남은 홍보 시간 15분 남았습니다」.
  판정 순서: 화면 노래 말 → True · 화면 발화 말 → False · 그 외 단위 문장.
- **짧은 span 의 화면 증인**: 훅 자막 「5호 30초입니다」(청취 「홍보 30분 남았습니다」, 화면 「홍보 시간이 30분 남았다고
  알린다」)가 ① 어절 단위는 숫자 섞인 어절을 안 봐서(한글 전용 조건) ② span 단위는 유사도 0.44 < 0.5 라 둘 다 놓쳤다.
  → `arbitrate_scene` 은 한글·숫자 어절(`_HANGUL_DIGIT`)도 보고, `scene_backed_heard` 는 **whisper ≤3어절 · 화면 어간
  ≥2 · 청취 ≤2배** 일 때만 하한 `SCENE_SIM_MIN_MULTI` 0.35(「50표!」→8어절 문장 같은 욱여넣기는 2배 조건이 막는다).
  `word_subtitles` 는 span 폴백을 **교정 전 whisper** 로 재고(어절 교정이 「홍보」를 넣으면 그 어간이 증거에서 빠져
  「홍보 30초입니다」 반쪽으로 끝난다), 어절 교정이 있었어도 짧은 span·어간 2개면 span 채택이 이긴다.
  드라이런: 8화 전편 span 채택 81→94 · ep8ex01 94→103(추가분은 「서진아, 세상이 노랗다」 류 짧은 문장).
- 회귀 가드: `tests/test_v3_singing_skip.py`(+1) · `tests/test_v3_heard_priority.py`(+1). 전체 2202 통과.

### 「15분 남았습니다」 — 화면 증인의 발음 닻 · 오버라이드 cue 승계 결함 (2026-09-09 밤, 사용자 지적)

- **발음 닻**(`assemble.PHONETIC_ANCHOR_MIN` 0.5 / `_SHORT` 0.6 · `textcheck.phonetic_sim`·`jamo_seq`·`read_digits_ko`):
  실제 발화 「자 여러분 이제 15분 남았습니다 15분 안에 빨리 다 뿌리셔야 돼요」를 whisper 는 거의 그대로(「나왔습니다」·
  「세계란에」만 오류) 냈는데, 청취는 「남은 홍보 시간 15분 남았습니다」로 **요약**했고 화면 묘사도 같은 호출의 같은
  요약(「남은 홍보 시간이 15분이라고 알리자」)이라 어간 남은·홍보·시간이 전부 '증거'로 잡혀 span 채택이 verbatim 을
  덮었다. **청취와 화면 묘사는 같은 Stage 2 호출의 산물이라 서술어에서는 독립 증인이 아니다.** 화면 증인은 whisper 가
  *잘못 들은* 단어를 바로잡는 것이지 모델의 다른 문장을 들이는 게 아니므로, 어간을 든 청취 어절은 whisper 어절 중
  **발음이 닮은 것**(자모 나열 difflib · 숫자는 한국어 읽기 — 삼백장↔산맥장 0.78 · 고무줄밖에↔꼬물들밖에 0.72)이
  있어야 한다. 2음절 이하는 0.6(「시간」↔「세계란에」 0.57 이 통과하던 자리). 예외 `missed_words`(바로 앞이 무성 조각 +
  청취가 더 김 — 「저희 300장 들고 왔잖아요」)만 닻 없이 채택. 짧은 span 의 '증거 2개'는 어절 교정 수 + span 어간 수의
  합(「홍보」는 어절이, 「30분」은 span 이). 드라이런: ep8ex01 span 채택 103→76 · 8화 94→73.
- **오버라이드 cue 승계**(`overrides.apply_overrides_to_plan`): 자막 한 줄 고친 재렌더에서 **훅 내레이션이 통째로
  드랍**됐다. 붙잡은 덮개(hold)는 편집본 길이(duration_sec)가 소스 창보다 hold_sec 만큼 길어 `start+duration` 이 클립
  끝을 넘고 kind="end" 조회가 None. cue 가 든 `source_end_sec` 을 소스 끝 정본으로(없는 옛 cue 는 종전 규칙).
  회귀 가드 `tests/test_v3_overrides_anchor.py`(+1) · `tests/test_v3_heard_priority.py`(+1). 전체 2204 통과.
- ⚠ v3 자막 오버라이드는 **전량 교체**라 규칙이 바뀌면 오버라이드 파일도 새 세그먼트로 **재생성**해야 한다(옛 파일은
  옛 규칙의 문구를 얼려 둔다). ep8ex01 은 `scripts/` 없이 세션 스크립트로 재생성 — 편집실 왕복이 없는 로컬 잡의 임시 절차.

### ep8ex02 첫 렌더 검수 후속 2건 (2026-09-09 밤, 사용자 지적)

- **정지(hold)는 자료화면 종류에만**(`cover.INFO_SCREEN_KINDS` = 메시지/기사/댓글/문서/검색 · `is_info_screen`): 훅 덮개가 방송
  텔롭(「떡볶이 ♥너무 맛있어요♥ … 서진팀은 100장도 고군분투 중」, 정독 종류 '기타') 위에서 0.68s 정지해 "첫 장면
  뒷부분이 정지화면"이 됐다. 정독 패스가 종류를 매긴 조각은 그 종류로만 판정하고(기타 = 텔롭·로고·예능 자막), 종류 없는
  초벌 조각은 종전대로 글자 유무. `story.build_span_index` 가 `screen_text_kind` 를 싣는다(additive).
- **화면 묘사의 노래 근거는 단어만**(`assemble.SCENE_SING_WORDS` · `SPEECH_HINT` 확장): 「관객들이 '최수호 최고다'를 연호하고
...
### 화자 고정의 표본 재적률 · 컷 분할 (2026-09-10, 지금불륜 ep01x01 「대원 여러분」 실사고)

`finalize.SPEAKER_PRESENCE_RATIO`(0.5) · `hold_keyframes(scene_cuts=)` · `read_scene_cuts`.
- 실사고: 편집본 3~10초(원본 302.8~311.5) 와이드 숏 — 가족 셋이 x≈1300 에 7초 내내 서 있는데(작고 입 움직임 거의 0)
  마지막 1.5초에만 잡힌 촬영감독 뒤통수 "얼굴"(x≈360 · 표본 4/15 · talk 0.28)이 talk×√w 로 이겨 run 전체 x=372 →
  클램프 500. 말하는 가족은 오른쪽 가장자리로 밀렸다. 종전 재적 조건 `n ≥ int(표본×0.3)` 은 15표본에서 4 를 통과시켰다.
- **run 의 화자 후보는 그 run 표본의 절반 이상(ceil)에 있어야 한다.** 절반을 채우는 사람이 없을 때만 전원 폴백(종전 순위).
  ⚠ 트레이드오프: 화자가 고개를 돌려 검출이 절반 아래로 떨어지고 상대가 절반 이상 잡히면 상대를 고른다 — 그 케이스는
  아직 실측 없음(생기면 화자 기억·Stage 2 subject_pos 를 증인으로).
- **컷에서 run 을 쪼갠다** — grid `scene_cuts` 가 run 안쪽(양 끝 0.2s 제외)에 있으면 조각마다 x 를 따로 잡는다(클로즈업→
  와이드에서 같은 화자라도 얼굴 위치가 다르다). 컷 자리 한 프레임 점프는 안 보인다. 조각은 짧아도 앞뒤와 합치지 않는다.
  render_final 은 `read_scene_cuts(output_dir)` 로 grid.json 을 읽는다(없으면 종전).
- 재실측(같은 클립 오프라인): 302.9~303.25 · 303.25~309.78 · 310.36~311.25 전부 x=1302.5(가족). 회귀 가드 3건
  (`test_v3_text_style.py` 끝).
- 곁다리(같은 편): Stage 2 기록이 엔딩에서 **한 조각씩 밀려** 있었다(골드버튼 클로즈업 실측 353.06~358.19 · 북어 358.19~362.0 ·
  소파 362~389.75 인데 기록은 각각 다음 조각에). 덮개 프로브가 "골드버튼 없음·소파" 로 모순을 잡았지만 되돌림 상한(1)에 걸려
  그대로 나갔다 → `checkpoint_story.json` 엔딩 비트 손편집(`flow.user_edits`). 이 밀림은 별건 후보(시각 정합 벨트는 경계만
  보고 **내용 귀속**은 안 본다 — 프로브 모순이 남으면 인접 조각의 화면 묘사를 대조해 자동 되돌리는 층이 없다).
- **검출기 층의 진짜 원인(같은 날 후속)**: 위 재적률·컷 분할로 뒤통수 둘 중 오른쪽(x≈1300)을 고르게 됐을 뿐, 가족은 여전히
  중심이 아니었다(실측 x≈892). `reframe._YuNetDetector` 가 **40px 미만 얼굴을 버려** 와이드 숏의 가족(27~29px · 점수
  0.90~0.94)은 아예 없었고, 촬영감독 뒤통수(96~136px · 점수 0.66~0.82)만 남았다. 점수로는 못 가른다(83.5s 박경희 큰 얼굴
  0.66 · 누운 경희 0.78). **YuNet 랜드마크가 뒤통수에서 붕괴한다**(눈 간격·입 폭 ≤0.03 vs 실제 얼굴 ≥0.17 — EP01 9프레임
  실측) → `reframe.yunet_plausible`(크기 `YUNET_MIN_FACE_PX` 20 + 랜드마크 폭 `YUNET_MIN_LANDMARK_SPREAD` 0.08).
...
- 실사고: 숏/리버스숏 — 김혜수(화자)는 오른쪽 초점 안(w140 · 선명도 10.6~12), 조여정은 왼쪽 전경 초점 밖(w360 · 5.8).
  흐린 전경이 크고 입 움직임도 잡혀 talk×√w 를 이겼고, 화자 기억(직전 숏의 x≈570)도 그쪽을 가리켰다. 장면 전환 검출
  (0.3)이 이 숏 전환을 놓쳐 컷 분할도 못 막았다.
- 규칙: 표본 안 가장 선명한 얼굴 대비 0.6 미만인 사람은, 초점 안 사람이 하나라도 있을 때 후보에서 뺀다(기억 `near` 도
  남은 후보에서만). 다 비슷하면 종전 순위. 5-튜플 구 표본(캐시)은 판정 없음 = 종전과 동일.
- 같은 편 손편집: beat4 before 덮개를 찌라시 기사 화면(sp1048 「류지수 불륜남 A씨」 2256.75~2259.07)으로(사용자 "자료화면이
  관련 기사면"), 제목 오버라이드 「가족 이미지로 투자받고 / 불륜 찌라시에 날아감」(사용자: 별장이 아니라 가족 이미지 투자 →
  찌라시로 철회 위기에 초점). 원본 `checkpoint_story_pre_cover_fix.json.bak`.
- 회귀 가드: `tests/test_v3_text_style.py::test_rank_speaker_x_drops_out_of_focus_face`.
- **오버라이드 cue 승계도 같은 비트 먼저**(같은 날, 「앞집 수정의 인맥을」 드랍): 덮개 소스 창 끝(1532.2)이 앞 비트 클립
  (1532.05~1539.0)에도 들어 있어 전체 타임라인 첫 일치를 끝으로 잡아 시작보다 앞서 버렸다. `overrides.apply_overrides_to_plan`
  이 `finalize_cues` 처럼 cue `beat` 의 클립을 먼저 본다(`tests/test_v3_overrides_anchor.py` +1).
- 제목 말투 다양화 프롬프트는 넣었다가 사용자 지시로 **뺐다**(2026-09-11 — 모델이 한 편만 보므로 지시만으로는 겹침을 못 막는다).
  프롬프트는 12자 규칙 판 그대로. ep01x02 제목 오버라이드 「완벽 가족으로 받은 투자 / 찌라시 한 장에 위기」는 유지.

## 쇼츠 구성 규칙 5건 — 4편(ep01x03) 검수에서 나온 사용자 지시 (2026-09-11) — 같은 날 저녁 코드 반영(아래 절)

> "쇼츠에서 중요한 건 초반 3초고, 이 장면을 고른다는 게 그 3초가 재밌다는 건 아니잖아?"

1. **초반 3초는 대사다.** 자극적인 *사건*(「남편을 뒤쫓았는데 시체가 있었다」)을 골랐다고 그 첫 3초가 재밌는 게 아니다 —
   내레이션 「남편을 뒤쫓던 아내」 위에 주인공이 걸어가는 평범한 컷은 훅이 못 된다. 훅은 **강한 한마디**(「너 바람피니?」)로
   열고 내레이션은 그 뒤에. (hook_line 이 None 인 채 무성 목격 장면 + before 덮개로 시작한 것이 이번 결함.)
2. **라벨은 반응하는 사람에게.** 「(동공지진)」은 「너 바람피니?」를 *들은* 김지훈(임재홍)에게 붙어야 하는데 말한 김혜수에게
   붙었다. 라벨 identity = 그 순간 표정이 바뀌는 인물(대사의 청자), 화자가 아니다.
3. **시간 경과를 단정하지 마라.** 「시작은 며칠 전」 — 드라마상 당일일 수도 다음날일 수도 있다. 기록에 근거가 없으면
   되감기 표지는 「사실 시작은,」「이야기는 이렇게 시작됐죠」처럼 **기간 없는** 표지로.
4. **내레이션이 가리키는 것은 덮개 화면에 있어야 한다.** 「불륜 정황을 확인하자」는 무슨 정황인지 화면에 없어 흐름이 끊긴다
   — 추상어(정황·의혹) 대신 **화면에 보이는 것**(「남편 지갑에서 나온 건」 + 콘돔 발견 컷)을 말하거나, 그 장면을 덮개로.
5. **엔딩은 리빌이 아니라 반응 + 질문.** 「불륜 현장이 아니었죠」(+ 시체 컷)로 답을 주면 원본을 볼 이유가 없다. 주인공의
   반응 컷 위에 「과연 무엇을 본 걸까?」 — 시청자가 원본을 찾아보게. (결말 유보 엔딩의 표준형 — reveal=end 와 한 벌.)
- 참고 연출: 수작업 v10(`~/premiere_claude/shorts_ep01_v10`, 「너 바람피니?」 오프닝 — 강조 자막 EPOP + 상황 라벨
  「(자기 전, 침실)」 + 임팩트 효과음). 전체 스토리라인이 아니라 **초반 연출만** 참고(사용자).
- 이 편(ep01x03)은 위 규칙대로 `checkpoint_story.json` 손편집(원본 `checkpoint_story_pre_hook_fix.json.bak` → 2차
  `checkpoint_story_pre_rules_fix.json.bak`) + 내레이션 2줄 재합성. 프롬프트·검증기 반영은 별건(다음 편 전).
- **사람 지정 고정 크롭**(같은 날, 4편 훅 직후): 비트 `crop_x`(소스 px · 선택 `crop_y`) → assemble 이 클립 `reframe={"mode":"fixed",…}` →
  `finalize.fixed_crop_map` 이 추적·앵커 맵 위에 마지막으로 얹는다(그림 사각형·밴드 크롭 안 클램프). 계기: 「너 바람피니?」 직후
  남편은 뒤쪽 멀리(x≈570, 얼굴 ~20px)라 YuNet 0 검출 → 화자 고정이 김혜수 자리(1565)에 머물러 라벨을 붙일 남편이 화면에 없었다.
  v10 도 같은 컷을 cx 560 으로 손지정했다. 회귀 가드 `tests/test_v3_edit_fx.py::test_fixed_crop_from_beat_crop_x`.

### 구성 규칙 5건 코드 반영 — 사용자 교정판 (2026-09-11 저녁, 2화 착수 전)

위 5건을 프롬프트·검증기로 옮겼다. 교정된 것 둘: ① "초반 3초는 대사"가 아니라 **"첫 3초 안에 정지 없음"**(내레이션으로
여는 것 자체는 허용) ⑤ 엔딩 질문형은 **강제하지 않는다**(다양성). `tests/test_v3_composition_rules.py`(12건). 전체 2233 통과.

- **규칙 1 — `cover.opening_motion`**(순수·결정적): 편집본 첫 `OPENING_MOTION_SEC`(3.0s) = 비트 0 의 before 덮개(조각·쌓기·
  정지) → 비트 0 조각(head_trim 반영). 이벤트 = 조각 경계가 원본에서 불연속이면 컷 · grid 장면 전환이 조각 안(끝 경계
  포함 — 3.0s 컷은 '3초 안')이면 컷 · 유성 조각 시작은 발화. 정지(hold)가 3초 안에서 시작하면 그 자체가 위반. 위반이면
  걸음 4 재질의 사유("훅 문장 한 줄·컷 있는 화면·hold 금지")로 모순과 같은 통로를 탄다. 소진 뒤 잔존은 `audit.opening_static`
  + ⚠ 로그(실패는 아님 — 손편집 몫). LINES_PROMPT hook 역할·narration PROMPT 훅 규칙에도 같은 말.
- **규칙 2 — Stage 4 라벨 청자**: `label_events` L·G 이벤트에 `speaker`(자막 세그먼트의 화자)를 싣고 표에 「— 화자: X」로
  보인다. reaction 라벨은 `person`(그 순간 표정이 바뀌는 인물) 필수 · 화자 본인이면 드롭 · `person_visible:false`(얼굴이
  멀리·뒤돌아) 면 드롭(프롬프트: 대신 그 비트 `crop` 을 그쪽으로) · clip_characters 에 없으면 드롭. C 앵커(대사 없는 컷)는
  자유. `person` 은 entry 에 additive 로 남고 라벨 프로브 문맥("**X** 의 반응 — 그 얼굴이 보이고 맞아야 fit")에 실린다.
- **규칙 3 — `narration.TIME_ASSERT_BANNED`**: 며칠 전·그날 밤·다음 날·몇 시간 후·이튿날·일주일·어제·오늘 밤 … 어느 줄에든
  있으면 반려(형식). `REWIND_MARKERS` 에서 기간 어휘("시간 전"·"일 전"·"그날 아침"·"오전")를 뺐다 — 남은 표지는 전부 기간
  없음(테스트가 둘의 교집합 0 을 고정). 프롬프트 예시도 "사실 이 완판, 시작은," 으로.
- **규칙 4 — 내레이션 = 화면, 세 구멍 전부**:
  ① 전수 프로브 — 여유 없는 창(slack<1s)·뮤트·spill·정지·쌓기도 **시작 고정(`fixed_start`)** 프로브로 문장·화면 대조만 얻는다
    (`run_probe(fixed_start=)` → snap "fixed" · `_verify_probe`). 예산 `FLASH_BUDGET` 10→12 **패스당** 리셋, `probe_cache`
    ((tag, 문장, 창) → 결과)가 재배치 재호출을 막는다.
  ② 손편집 덮개 — pipeline `_run_m3` 가 story 확정 직후(캐시 로드 포함) `cover.verify_covers` 로 text_matches 없는 덮개를
    전수 검증하고 결과를 `cover.probe`(verify_only) 에 써서 **체크포인트를 다시 쓴다**(한 번만 든다). 모순은 run_log
    `cover_verify` + ⚠ 로그 — **제거하지 않는다**(손편집은 사람이 이긴다).
  ③ 되돌림 `COVER_REASK_MAX` 1→2, 소진 뒤 잔존은 **필수 자리(훅·점프 다리·무대사 비트)면 `ValueError`**, 선택 자리면 그 내레이션을
    빼고 나머지를 재배치(캐시로 재호출 0) → `narration_dropped`·`audit.narration_contradictions_dropped`. 종전 "검수 대상으로
    기록만" 은 폐지. 프롬프트에 추상어 금지("정황·의혹" 대신 "지갑에서 나온 건,") + "코드가 다시 보고 대조한다".
- **규칙 5** — narration PROMPT after_last 에 "결말 유보 편이면 반응 컷 위에 질문으로 끝낼 수도 있다 — 필수 아님" 한 줄뿐.
  reveal=end 결과 씬 게이트는 그대로.
- 미반영(별건): 훅 내레이션 필수(`required={0}`)는 그대로다 — "대사로 바로 여는" 편(ep01x03 r3)은 손편집 경로. 열려면 걸음 4
  required 를 풀고 opening_motion 이 발화 시작을 보게 하면 된다(검사는 이미 그 경우를 통과시킨다).

### 2화 1편(ep02full) 검수 후속 — 꼬리 트림 · 자막 마침표 · watch_trim 오판 고정 (2026-09-11 오후)

- **비트 `tail_trim_sec`**(`assemble` flush · head_trim 의 거울): 마지막 조각 안의 소스 절대초면 클립 끝을 당기고 `tail_trim`
  플래그(벨트가 이미 인정하는 키)로 등록. 사람 손편집 통로 — 훅의 5.6s 반응 조각을 통째로 쓰거나 빼는 길밖에 없던 것.
  범위 밖이면 무시(종전 동일). 가드 `tests/test_v3_text_style.py`(+1).
- **대사 자막 끝 마침표 제거**(`assemble.strip_dialogue_period`, 사용자 지시 "대사 자막에 '.'은 빼줘"): 줄 나눔이 문장
  힌트(마침표)를 다 쓴 **뒤** 최종 텍스트에서만 뗀다. `?`·`!`·`…`는 남긴다. v3 `word_subtitles` 두 경로. 자막은 LLM 이
  쓰는 게 아니라 전사·청취 텍스트라 프롬프트가 아니라 코드에서 뗀다(가드 +1). v1 무변경.
- **Stage 2 기록 밀림 5번째 사례(같은 편 · 사용자가 프레임으로 잡음)**: 03:02~03:19 기록이 **두 조각쯤 뒤로 밀려** 있었다 —
  sp0074(03:15.25~18.60)에 "주보성이 여성 뺨을 쓰다듬음"이라 적혔지만 실제 그 컷은 03:04~03:11(sp0072)이고 03:15~18.6 은 꽃병
  패닝 인서트, sp0075~79 "여성 얼굴 어루만짐·참담"은 실제 안내 데스크 직원 옆모습(03:19~03:23)이다. watch_trim 은 초안 픽셀을
  보므로 "풍경 인서트"·"케이스 정지 샷(02:39.5~02:42.75 실측 맞음)" 판정이 **옳았고**, 기록을 믿고 컷을 되살린 첫 손편집이 틀렸다.
  교훈: **기록과 watch_trim 이 갈리면 프레임을 뽑아 본다**(접촉시트 `ffmpeg fps=2,tile`) — 기록은 경계만 정합이고 내용 귀속은
  벨트가 안 본다. 편은 실측 시각으로 재편(뺨 03:08.5~11 + 충격 ~03:13 | 직원 옆모습 03:20.2~22.98 새 비트 | 옷장 리빌
  tail_trim 02:41.0). 사람 고정 통로(`checkpoint_watch_trim.json` cuts=[] · post_fingerprint 삭제)는 그대로 유효하되 이 편엔
  안 썼다(재편 뒤 watch_trim 지목 0).

### 최종본 H.264 level 6.2 → QuickTime 깨짐 (2026-09-11, ep02full 복도 컷 실사고 · 사용자 스크린샷)

- 증상: 완성본의 어두운 아웃포커스 구간(02:42.75 병원 뒤통수 컷)이 QuickTime 에서 매크로블록으로 깨짐. ffmpeg 소프트웨어
  디코드·프레임 QC·프레임 간 차분은 전부 정상이라 **렌더 검증이 못 잡았다**(같은 파일을 `-hwaccel videotoolbox` 로 디코드하면
  그 구간에서 `hardware accelerator failed to decode picture`·`-12909` 가 123프레임).
- 원인: libx264 자동 level 이 **6.2**(`level=62`)로 찍혔다 — 필터그래프 출력 프레임레이트를 x264 가 못 읽는 것으로 보임(avg_frame_rate
  594500000/24774773 · time_base 1/1000000). 창을 잘라 재인코딩 실측: level 4.0 → VT 실패 0 · `-level 6.2` → 83(재현).
- 수정: `renderer.H264_LEVEL="4.2"` — libx264 세 프리셋 전부 `-level 4.2`(1080×1920 ≤64fps 안). 하드웨어 인코더 인자는 그대로.
  가드 `tests/test_render_h264_level.py`. 검증 자: `ffmpeg -v error -hwaccel videotoolbox -i final.mp4 -f null - | grep -c failed` 가 0.
- ⚠ 이전 납품본(1화 3편)은 level 62 이지만 VT 실패 0 — 실패는 콘텐츠 의존(어두운 노이즈 구간). 재납품 여부는 사용자 판단.

### 2화 1편 스토리라인 v2 (2026-09-11 오후, 사용자 지시 — 손편집 절차의 표준형)

대사 훅(「이 아줌마는 누구야?」「아빠 친구」, 내레이션 없음) → 돌잔치(남부러울 것 없는 가족 · 「그런데 신랑의 눈은, / 딴 여자한테 가
있었죠」 — 남편 곁눈질·여직원 컷은 **B-roll 덮개로 순서 바꿔 배치**) → 딸 그림 → 옷장 → 병원(선명한 걷는 컷 · 남편+안경 여자 · 뺨 · 충격) →
「그 둘을 지켜보던 직원의 목엔,」 → 목걸이 줌 → 적반하장 엔딩. 손편집 방식: 내레이션은 `default_synth` 로 먼저 합성해 길이 L 을 얻고,
덮개 = `{position, kind:designated, t_in, t_out=t_in+L, span_ids(spans_overlapping), probe:None, hand:true}` · cue = 기존 cue dict 모양
(source_time_sec = t_in+0.1 · source_end_sec = t_out · audio_path · measured_sec). 비트 순서는 자유(되감기는 내레이션 표지 「사실」로).
`verify_covers` 가 손편집 덮개 7개를 전수 프로브해 7/7 일치를 기록했다 — 손편집도 검증을 받는다. 자막 오버라이드는 새 타임라인에서
**재생성**(전량 교체 규약: 흘러든 한 단어 「내」 제거 · whisper 오인식 「하나 자꾸 자자」 교정).

### 강조 효과음 — 붙어 있는 강조는 다른 소리 · 줄별 끄기 (2026-09-11, 2화 3편 검수)

- `sfx_narration.place_emphasis_sfx`: 직전 강조와 `EMPHASIS_FOLLOWUP_GAP_SEC`(3.0s) 안이면 매니페스트 `emphasis_followup` 풀(punch·gunshot —
  "가벼운 효과음 뒤 다음 문장에서 연이어 강조")에서 **직전과 다른 계열**을 고른다(없으면 soft → label 풀). 떨어진 강조는 종전(`emphasis_soft` = hit3).
  종전엔 soft 풀이 hit3 하나라 강조 7줄이 전부 같은 소리였다. 실측: hit3 → punch → gunshot · hit3 → punch.
- `emphasis[].sfx: false`(finalize) — 그 줄은 글자·팝·줌만, 타격음 없음(사용자가 결국 전부 켜서 3편엔 안 씀 · 어휘는 남김).
- 강조 줄 번호 재확인 `finalize.resolve_emphasis_index`(같은 날 4편에서 두 번째 실사고 뒤 코드로 막음): Stage 4 강조는 **자막 줄 번호**로 붙어 오버라이드로
  줄을 나누거나 합치면 엉뚱한 줄이 빨개졌다(3편 L10·L23 → 「나 이미」「엄마 때문에」 · 4편 4줄→3줄 병합으로 한 칸씩). 항목의 `text`·`start_sec` 로 ①번호의 줄이
  같은 글자면 그대로 ②같은 글자 줄 중 가장 가까운 시각 ③(글자를 사람이 고쳤으면) 시작 시각 1.0s 안 가장 가까운 줄 ④못 찾으면 번호. `sfx:false` 도 같은 해석.
- ⚠ 렌더 지문은 효과음 선택 **코드**를 모른다 — sfx 로직만 바꾼 재렌더는 "지문 일치"로 옛 최종본을 그대로 둔다. `render_fingerprint.json` 을 지우고 돌린다.
- 가드 `tests/test_v3_edit_fx.py`(+4).

### 훅 화면은 눈길을 끄는 조각 — `HOOK_COVER_MIN_IMPORTANCE` (2026-09-11, 2화 4편 사용자 지적)

- 규칙 1(첫 3초 컷·발화)은 통과했지만 훅 내레이션 밑이 옥상 와이드에 서 있는 인물(Stage 2 importance 2)이라 눈길을 못 끌었다.
  걸음 4 「쓸 수 있는 화면」 목록에 importance ≥4 조각을 **★** 로 표시하고, 프롬프트가 훅 화면을 ★ 에서 고르라고 한다
  (서 있기·걷기·뒷모습·풍경 와이드 금지). `validate_narrations(span_index=, hook_before_t=)` 가 before_beat 0 의 cover 최대 importance 가
  기준 미만이면 반려(후보 ★ id 를 사유에 싣는다) · 화면 미지정도 반려. 기준 = min(4, 훅이 실제로 쓸 수 있는 화면 — 훅 첫 조각보다 앞 — 중 최고값)
  이라 ★ 가 없는 편은 막히지 않는다. span_index 미지정(구 호출)은 검사 없음. 가드 `tests/test_v3_composition_rules.py`(+2).
- 4편 손편집: 훅 화면을 소화기 분사(823.42 · imp 4)로 교체 → 분사 직후 「큰일 날 뻔했네요」로 이어지고, 뒤쪽 분사 비트는 뺐다(61.0s).
- 사용자 교정(같은 날): **재미 기준은 첫 화면에만** — 나머지 내레이션 덮개는 관련성·자연스러움이 우선. 프롬프트에 "★ 는 첫 화면에만"을 명시하고
  LINES_PROMPT hook 역할에 "재미·시선이 최우선". 검증기는 원래 before_beat 0 만 본다.
- 라벨 프로브(`stage4.LABEL_PROBE_PROMPT`)에 "누가 무엇을 했는지 틀리게 말하거나 그렇게 읽히면 fit=false" 추가 — 4편 「(본인한테 쏨)」(남이 쏜
  소화기를 스스로 쏜 것처럼 읽힘)이 표정 타이밍만 보고 통과했던 실사고. 그 라벨은 checkpoint_style 에서 손으로 뺐다.


## 썸네일 안전 구역 (2026-09-11, 사용자 지시)

> "앞으로 이 썸네일 안에 제목/영상화면/작품 로고가 다 들어오게 해야할 것 같은데" ·
> (가왕쇼 네 편 스크린샷) "맨 위와 맨 아래 경계 자체는 참고할 만 해"

### 썸네일 안전 구역 자동 맞춤 (`finalize.fit_thumbnail_safe_zone` · v3 전용)

- 실사고: 지금불륜 2화 2편 썸네일에서 작품 로고 둘째 줄이 잘렸다 — 로고가 밴드 아래 남은 공간 **가운데**
  (1565~1831)에 앉았다. 썸네일은 2:3 가운데 크롭이라 캔버스 약 148~1770 만 보인다(스크린샷 두 장 실측 ·
  정본 `docs/shorts_thumbnail_safe_zone.md`).
- 경계(블록 기준): 제목 블록 윗변 ≥ `THUMB_SAFE_TOP` 180 · 하단 블록(플랫폼 줄·작품명/로고·캡션) 아랫변 ≤
  `THUMB_SAFE_BOTTOM` 1715 — 사용자가 기준으로 지목한 가왕쇼 템플릿(187 · 1711)이 안에 든다.
- 렌더 직전(로고 해석 뒤 · 제목 폭 맞춤 **앞**) 한 번. 안에 들면 design 그대로(회귀 0). 밖이면 ① 로고 가운데
  정렬 해제(밴드 +20) ② 반대쪽 여유만큼 밴드 이동 ③ 로고 축소(하한 `LOGO_MIN_SCALE` 0.7) ④ 제목 상한 축소
  (하한 `TITLE_MIN_SCALE` 0.8) 후 ② 다시 → 그래도 밖이면 `[v3/썸네일] ⚠` + `unmet`(렌더는 계속 — 템플릿 몫).
- ⚠ 제목은 이 편의 실제 크기가 아니라 **채널 상한(`title_sizes`)** 으로 잰다 — 편마다 밴드·로고 자리가 같아야
  채널 격자가 가지런하다. 실제 제목은 `fit_title_sizes` 가 상한 이하로만 줄여 늘 판정보다 안쪽이다.
- 판정은 `layout_extents` — render_final 과 **같은 함수**로 잰다. 그래서 자막 기준 margin 계산을
  `base_text_margins` 로 떼어 냈다(render_final 인라인 코드와 산출 동일 — 로그 문구도 같다). 번인 회피는 판정에
  안 넣는다(자막을 올리기만 해서 작품명을 덜 민다 → 판정이 보수적).
- 밴드를 옮기면 Stage 4 라벨(캔버스 좌표 y)도 같은 px 만큼 옮긴다(`_label_dy`). 크롭·줌·fit 은 밴드 내부라 무관.
- 결과: 지금불륜 밴드 443→434 · 로고 620×194→486×154 · 하단 1831→1714. 가왕쇼 밴드 420→424 · 제목 상한 92/112→87/106.
- 기록: run_log render `thumb_safe` {safe, title_top, bottom, band, actions, unmet, band_shift}(최종 실제 값).
  렌더 지문에 경계 상수가 들어가 **기존 잡은 다음 재개에서 한 번 다시 렌더된다**.
- 미리보기: `outputs/지금불륜이문제가아닙니다_ep02x02_thumbsafe`(2화 2편 클론 · `--from-step render`). 납품본
  (`jigeum_ep01_*`·`jigeum_ep02_*`)은 옛 배치 그대로다.
- v1 렌더 경로는 이 맞춤이 없다(전 채널 배포 경로라 별건).
- 회귀 가드: `tests/test_v3_thumbnail_safe_zone.py`(9건).

### 업로드(메타데이터) 제목 — 엔진 몫 아님

- 업로드 제목은 올릴 때 **사람이** 화면 제목과 따로 짧게 쓴다(길면 카드 두 줄째에서 말줄임 — 「불륜 들키자
  적반하장으로 폭언 퍼붓는 남편 …」). 파이프라인에 생성 단계를 넣었다가 사용자 지시로 걷어냈다
  ("화면 상단 제목이 아니라서 v3 파이프라인을 수정할 필요는 없어"). `title.txt` 는 렌더러의 화면 제목 파일이다.

## 폰 재생 화면 안전 박스 (2026-09-18, 사용자 결정)

`app/v3/safe_zone.py`(상수·실측 도구) · finalize `enforce_safe_box`·플랫폼 오프셋·`emphasis_styles(font_path=)`·
렌더 뒤 인물 가장자리 점검 · story_flow 걸음 2 `title_len_rule`/`title_width_problems` · pipeline `title_fit_spec`.

- 실측: 가왕쇼 8화 어시장 편 **아이폰 재생 화면** 스크린샷 ↔ 완성본 — 재생 화면은 영상을 세로에 맞춰 채우고
  **좌우 약 47px** 을 자른다(긴 안드로이드 20:9 는 계산상 약 83px). 상단 아이콘이 약 200 까지, 하단 채널명·설명이
  약 1620 부터 덮는다. 실사고: 밴드 모서리 24px 에 있던 「티빙」의 「빙」이 잘렸고 제목이 58~1021px 로 끝까지 찼다.
- **박스**: 글자 요소 가로 `SAFE_X0 100 ~ SAFE_X1 980`(오른쪽 **마지노선 980**) · 제목 윗변 `SAFE_TOP 200`
  (종전 썸네일 기준 180 · 2px 허용) · 핵심 자막 아랫변 1600(정보) · 썸네일 아랫변 1715 는 그대로.
- **제목은 글자 크기 고정, 글자 수를 줄인다**(사용자 결정 — "글자 크기를 줄일 게 아니라"). 걸음 2 가 채널 제목
  폰트로 **상한 크기(92/112)** 그대로 재서 880px 을 넘으면 반려한다(프롬프트 안내 = 윗줄 한글 약 9자·아랫줄 약 7자).
  `title_fit` 미지정(legacy·테스트)이면 프롬프트·검증 종전과 바이트 동일. 렌더의 `fit_title_sizes`(폭 880)는
  손으로 넣은 제목이 넘칠 때의 **최후 수단**이고 `⚠ 제목 N줄 … 글자 수를 줄여야 한다` + run_log `title_overflow` 로 크게 알린다.
- 위 경계 200 은 **로고를 먼저 줄여**(하한 0.7) 밴드를 내려서 맞춘다 — 제목 상한 축소는 그래도 모자랄 때만(지금불륜·
  가왕쇼 둘 다 92/112 유지 실측). `fit_thumbnail_safe_zone` ③ 이 위쪽 부족에도 작동하도록 바뀌었다.
- **마지노선은 모든 글자·로고에**: 밴드 모서리 플랫폼 표기는 오프셋을 끌어올려 바깥 끝이 캔버스 끝에서 100px
  (가왕쇼 TVING 로고 x 800~980) · 가운데 정렬 요소(작품 로고 폭·로고 아래 문구·작품명 위 플랫폼 줄·작품명 글자)는
  `enforce_safe_box` 가 880 을 넘으면 그 요소 크기만 줄인다(템플릿 고정 문구라 글자 수를 못 바꾼다) · 라벨은
  `label_x_range`·`avoid_faces` 가 글자 끝을 100~980 안으로 · 강조 자막은 배율만 낮춰 880 안으로.
- **영상 밴드는 그대로 꽉 채운다**(사용자 결정) — 대신 렌더 직후 완성본을 1fps 로 훑어 **인물 얼굴이 잘림 띠에
  30% 이상 걸린 구간**을 `[v3/안전구역] ⚠ …` + run_log render `edge_faces` 로 알린다(YuNet + 랜드마크 타당성 ·
  완성본 프레임 기준이라 크롭 방식 무관). **자동으로 크롭을 옮기지 않는다** — 고치는 건 손편집(비트 `crop_x`)·컷 선택.
  실측(가왕쇼 9화 게릴라 무대): 투샷 끝의 윤수현 3건 + 관객석 가장자리 5건, 전부 실제 얼굴. Stage 4 줌 지시에도
  "좌우 약 100px 잘림 — 인물이 가장자리로 밀리지 않게 앵커" 한 줄.
- 가왕쇼 템플릿: 플랫폼 표기 「티빙」 글자 → **TVING 로고**(`app/assets/logos/tving_logo.png`, 투명 PNG 2000×505 ·
...
## 로또 1-2화 합본 staged 첫 실런 후속 — 선공개 선두 · 제목 글자 수 · 배속 먼지 · TVING 로고 (2026-09-21~22, 사용자 지시)

- **선공개 영상은 합본 맨 앞**(`scripts/concat_source_clips.py` `--prerelease-regex`(기본 '선공개')·`--prerelease-ids`): `--ids` 어디에 있든
  머리로 옮기고 회차는 단정하지 않는다(`episode:null`·`episode_label`). 선공개 = 예고편이 아니라 본방 전날 **본편 한 장면을 통째로** 푼 것
  (사용자 정정) — 안의 흐름은 본편 그대로, 다만 본편 어느 시점인지 모른다. 새 산출 `source_notes.md`(클립 경계표 + 선공개 경고)는 제작 가이드
  형식이라 `--guide` 로 함께 넘긴다(⚠ `--guide` 를 주면 자동 탐색이 꺼지므로 작품 가이드도 같이). tikitaka 는 manifest 를 읽지 않는다.
  분석(Stage 1·2)은 가이드 본문을 안 받지만 선공개 구간이 제외되지 않음을 실측(제외 창 0). 되감기 표지(`bridge_marks`)는 합본 시각 계산이라
  선공개↔뒤 클립 사이에 "사실 시작은" 표지를 요구할 수 있다 — 미해결 별건.
- **제목은 처음부터 글자 수로**(`title.title_prompt(title_fit)` · cli 가 `title_fit_spec`(v3 와 같은 자)을 `guide["title_fit"]` 에 싣고 sha 에 포함):
  '총 24자 안팎' 대신 템플릿 제목 폰트·상한 크기로 계산한 **줄당 글자 수**(로또 66/84 → 13/10자)를 뼈대·리빌딩·확인 패스 프롬프트에 싣는다.
  사용자 결정 "폭 검사를 넣는 게 아니라 처음에 글자 수를 정해야". 프리셋 없는 실행은 프롬프트 바이트 동일. 실측: 줄당 12~19자 → 7~10자,
  렌더 제목 축소·경고 0. 회귀 가드 `tests/test_tikitaka_title_budget.py`.
- **배속 1.2 먼지**(`grid_table.py`): `(끝−시작)/길이` 되계산이 `1.200000000000017` 을 내 `clip_len` 이 렌더 직전에 죽던 결함(clip01 메모의
  엔진 결함 ②) — 1.0 먼지 정리 옆에 1.2 도 같은 방식(abs 1e-9). 진짜 >1.2 는 여전히 거부. 가드 `test_tikitaka_cut_guard.py` 끝.
- **톤 가이드는 지양 단어 목록이 아니라 강조 서술로**(사용자: "기계적인 처리 말고 처음부터 잘 쓰게 명시") — `guides/tikitaka/로또…md` 톤 절을
  최우선 규칙(왜·방향·고쳐 쓰기 예시·자기 점검)으로. 실측: 「참교육한」류 0건, 제목이 '직장인 상황' 틀로.
- `lotto_tving_v2` 플랫폼 표기 「티빙」 글자 → TVING 로고(가왕쇼와 같은 값). tikitaka 렌더는 v3 `render_final` 을 쓰므로 폰 안전 박스가 그대로
  적용된다(실측: 로고 폭 960→880 · 플랫폼 오프셋 100px).
- ⚠ **`--tag` 변형은 뼈대(`outline_{tag}.json`)도 새로 만든다**, 그리고 확인 패스의 `speaker_fixes` 가 `transcript.json` 에 되써져 뼈대 지문이
  바뀐다 — "같은 대본으로 속도만" 비교하려면 원 `outline.json` 의 data 에 현재 지문을 얹어 `outline_{tag}.json` 으로 둔다(fast11 실측 절차).
  digest 는 매 실행 재생성됨(sha 가 실행마다 다름 — 원인 미조사 별건, outline 지문엔 bool 만 들어가 연쇄 없음).
- **내레이션 속도 기본값 `fast`(1.1배)** — tikitaka `--speed` 기본 normal → fast(2026-09-22 사용자 결정, "지금 영상이 아니라 앞으로의
  파이프라인 속도"). 로또 1-2화 very_fast(1.2배) 실청취 "생각보다 빠르다" · fast11 재실측(v3 48.3s·v6 69.1s — 같은 뼈대에서 내레이션이
  느려진 만큼 편이 길어진다). 가왕쇼(normal 명시)·지금불륜(fast 명시)은 스크립트가 값을 주므로 무관. ⚠ 속도는 staged 걸음 ② 지문에
  들어가 문장·화면을 다시 쓴다(렌더만 재실행되는 값이 아니다).
- **컷 가드는 켜되 기록만**(`--cover-cut-guard`, 2026-09-22 사용자 결정): 미해결 덮개(`needs_reselection`)가 있어도 렌더를 막지 않고 원 컷
  그대로 내보내며 `[cut-guard] ⚠` + run_log `cut_guard_unresolved` 로 남긴다(fast 속도에서 3편 중 2편이 0.2~0.3s 잔존 샷으로 막혔던 것).
  로또 실행 스크립트는 이 플래그를 켠 채로 둔다. 상세 `docs/CUT_GUARD.md`.

### 확인 패스 재작성 규율 · 관점(`관점:`/`--pov`) · 수리 기준 시각 (2026-09-22, 로또 fast11 v6 「침대에 누워 …」 실사고)

- **실사고 경로**: 걸음 ② 훅 「팀원들이 일찍 밥 먹으러 간 순간,」(게장집 화면) → 확인 패스가 응답을 출력 어휘(`{"type":"N","text","effect"}`)대로만 내
  **N 9개의 production_plan 을 전부 누락** + 문장 전부 재작성(0/6 보존) → 게이트 수리가 계획 없는 N 의 기준 시각을 **0.0초로 폴백**해 소스 맨 앞
  조각(선공개 침대 장면)을 후보로 → 프로브 불일치 → 2차 재작성 규칙("본 화면으로 문장을 다시 써라")이 「침대에 누워 로또 복권을 들여다보는 남성.」을
  만들고 통과. 실측: 정상 편은 확인 패스가 24문장 중 22 를 글자 그대로 보존(양자택일 — 어긋나면 통째 재작성).
- **확인 패스 프롬프트**(`verify.VERIFY_PROMPT`): 규칙 6 "절반은 의견형(드립·팩폭), 해설 위주면 바꾼다" **삭제** → "문제 없는 문장·제목·S/A 는 글자
  그대로, 바꾼 건 전부 changes 에" 보존 규칙. 규칙 7 은 처리 방법 명시 — 뒤 대사를 소개할 땐 **내용 대신 지시어**("그러자 A 가 이렇게 말합니다").
  출력 어휘에 `production_plan`. `verify.inherit_plans` — 응답 N 에 계획이 없으면 초안 계획 승계(같은 문장 → k번째 N 순).
- **수리 기준 시각** `staged.neighbor_anchors`: 계획 없는 N 은 앞 항목 끝·뒤 항목 시작을 기준으로 후보 100개(0.0 폴백은 S/A 가 없을 때만).
- **관점은 사람이 적는다**(사용자 결정 — digest 자동 추정 없음, 없으면 종전 = 사건 자체): 가이드 구조 키 `관점:`(작품 단위) · CLI `--pov`(편 단위,
  가이드보다 우선) → `guide["pov"]`(sha 포함) → `guide.pov_rule` 한 문장이 뼈대·리빌딩·확인 패스(guide_block)와 재순위(`RERANK_PROMPT`
  `{pov_note}` ⓪ 기준)에 **같은 문장**으로. 계기: 로또 v6 제목 「반차 낸 팀장에게 걸림」이 팀원(부하) 공감 프레임 — 톤 절("응징 틀 금지·상대역도
  같은 직장인")이 과교정을 유도했고 모델은 작품 축(팀장 vs 삐딱선 대리)을 알고 있었다. 로또 가이드에 `관점:` 한 줄. ves 대시보드 입력칸은 별건.
- ⚠ 가이드 sha 가 바뀌므로 기존 잡의 뼈대·걸음 ② 캐시는 재생성된다. `REBUILD_PROMPT`(single 흐름·staged 가 빌리는 내레이션 원칙 절)의
  "설명만 하는 해설 금지" 는 **쓰는 단계**의 문구라 손대지 않았다 — 별건.
- 회귀 가드: `tests/test_tikitaka_pov_verify_rules.py`(5건) · `test_tikitaka_stages` 의 프롬프트 검사 갱신.
- **쓰는 단계도 같은 취지**(같은 날 사용자 지시): `prompts.REBUILD_PROMPT` 제3원칙의 "설명만 하는 해설 금지 · 절반은 리액션·의견(드립·팩폭)"
  배정을 없앴다 — 말투는 [제작 가이드]가 정하고 없으면 장면에 맞게, 해설 위주여도 된다, 의견형은 선택. 뒤 대사는 지시어로. staged 걸음 ①·②가
  이 절을 빌려 쓰므로(`prompt_sections`) 두 흐름 다 적용. ⚠ 프롬프트 변경이라 기존 잡의 리빌딩·뼈대·걸음 ② 캐시가 재생성된다.
- **캐시 지문은 화자 표기를 보지 않는다**(같은 날 사용자: "같은 job 으로 취급하고 있어야"): `staged.speaker_neutral`·`items_neutral` ·
  `outline_fingerprint`(소스 스크립트를 화자 없이 다시 만들어 센다) · `script_fingerprint`(항목 speaker 제외) · `digest.digest_sha` 화자 제외.
  실사고: 확인 패스가 되쓴 화자 4줄(L-234~236 양준호→정선혁 · L-263)에 같은 태그 재실행이 뼈대·걸음 ② 14편을 통째로 다시 만들어 사용자가
  고른 편(v8·v3·v4·v7·v10)이 다른 대본이 됐고 14:21 판 대본은 덮어써 복구 불가. digest 가 매 실행 재생성되던 미조사 별건의 원인도 이것.
  대사 텍스트·가이드가 바뀌면 여전히 다시 만든다(테스트 고정). single 흐름 rebuild.json 은 원래 지문이 없어 무관. ⚠ `grid_table` 지문은
  그대로 transcript 통째(화자 교정 → 그 편 테이블·자막 색 재구성 — 의도된 동작).
- ⚠ **한 잡 폴더를 두 세션이 동시에 쓰면 안 된다** — 다른 세션의 `_shotfix`·`_endingfix`·`_polish` 변형 실행(확인 패스 포함)이 같은
  transcript.json 을 되쓴다. 변형은 태그가 달라도 전사·색인·digest 는 공유 파일이다.

### 뼈대 `context` — 처음 보는 사람을 위한 맥락을 걸음 ②로 넘긴다 · 검사 없음 (2026-09-22, 사용자 결정)

계기: 로또 fast11pov v1 「선배가 대리이고 / 후배가 팀장일 때」 — 공은태가 **왜 이번엔** 참지 않는지(13억 → 세탁소에서 화 못 내는
자신 → 반차 후 무단 외출)가 대본에 0건. 재료는 전부 있었다(소스 스크립트 28장면 · digest 줄기 「13억 로또 당첨과 태도 변화」 ·
가이드 `관점:`). 새던 자리는 걸음 ①→② 이음새: ①은 S·A 순서만 정하고, ②는 고른 장면(SC-020·022)의 화면 표 + **그 장면으로 좁힌**
scene_notes 만 받았다 — "왜 이번엔"의 근거가 문장 쓰는 걸음에 없었고, 화면 없는 이유는 프로브가 막았다.

- 걸음 ① 산출에 `context {why, scene_ids(≤CONTEXT_MAX_SCENES 2), how}` — "이 쇼츠로 작품을 처음 접하는 사람이 알아야 할 이 편 밖의
  이유"와 그것이 **보이는 장면**. `normalize_context` 는 모양만(없는 장면 빼고 기록·상한) — **거르지 않는다**(사용자: "검사 단계에서
  거르지 말자, 편집실에서 편집하면 된다"). 비어 있어도 통과.
- 걸음 ②: `scope_scenes` 가 context 장면을 화면 표에 넣고(그래야 그 이유를 화면 위에 깔 수 있다), 머리에 「이 편의 맥락」 한 줄
  (`context_block`), 규칙 "한 자리에서 확실히 — 짧은 조각 여럿 금지"(사용자: "너무 짧게 들어가면 더 정신없다"). 작품 이해는 **회차
  전체** scene_notes 를 싣는다(좁히기 폐지). 확인 패스 `carry_slots` 가 hook 처럼 승계.
- 지문: SCHEMA v3→v4(뼈대·걸음 ② 캐시 전부 재생성) + script 지문에 context. 회귀 가드
  `tests/test_tikitaka_staged.py::test_outline_context_reaches_step_two_without_a_gate`.
- ⚠ 제1원칙의 "보는 사람이 자연스럽게 이해되는 쇼츠"는 프롬프트 문장으로만 있고 어떤 검사도 안 본다 — 그것이 의도다. 이해되는가는
  편집실·사람 검수의 몫.
