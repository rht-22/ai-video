...
- 곁다리: L4 실패 메시지가 **원인 갈래를 구분한다**(`rerender.cut_mismatch_hint`) —
  **비디오 스트림끼리** 같으면 '컷은 재현됐고 차이는 오디오뿐', 다르면 'gen_flags 재현
  실패'. 종전에는 둘 다 후자로 뭉뚱그려 엉뚱한 곳(노브·소스)을 뒤지게 했다.
  ⚠ **첫 판(같은 날)에 단위 착오가 있었다** — ko 의 **컨테이너** 길이와 ja 의 **비디오
  스트림** 길이를 맞댔다. 두 파일 다 오디오 꼬리를 갖고 있으면 컷이 멀쩡해도 늘 '재현
  실패'로 읽힌다(실측: ko 컨테이너 39.400 vs ja 스트림 25.025 → 오판). 지금은 양쪽
  **스트림**을 비교하고 **두 숫자를 모두 문장에 적는다** — 판정이 틀려도 사람이 원본
  숫자로 되짚을 수 있어야 한다.
