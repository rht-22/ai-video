...
EXPECTED_DIFFS = {
    ...
    "dub.transcribe":
        "받아쓰기 상용구 환각을 거른다 — 2026-08-26 실사고(-wgxMaRHaYQ): 노래뿐인 14초 편에서 "
        "전 구간(0~14초)에 '다음 영상에서 만나요!' 한 문장을 지어냈고, 그것을 믿고 demucs 가 "
        "보컬(=노래 가사)을 지운 뒤 없는 말을 더빙·자막으로 박았다. vlp 의 방어는 "
        "no_speech·logprob 뿐인데 음악 위 환각은 모델이 자신 있게 내놓아 통과한다(백체크도 "
        "통과한다 — TTS 가 시킨 대로 말하므로 환각 검증이 아니다). 문구+길이 축을 더했고, "
        "문구만으로는 안 버린다(진짜 아웃트로가 있다). 대사가 안 남으면 더빙이 실패해 "
        "보컬 제거도 일어나지 않는다 — 원곡이 남는다. ai-video 가 앞서간 것.",
    "llm.resolve_model":
    ...
    "translate._transcreate_one":
        ...
        "또한 응답 색인을 _index_by_source 로 뺐다 — 모델이 프롬프트의 번호를 source 에 되돌려주면 "
        "(2026-08-24 실측: '1. L') 정확 일치가 전부 빗나간다. 프롬프트는 안 고친다(문구가 바뀌면 "
        "번역 결과가 달라져 vlp 와 대조가 안 된다).",
    "translate.translate":
        "재실행 캐시(P6-2): 원문이 같으면 있는 초벌을 재사용한다 — dub 의 번역 캐시와 같은 "
        "규약. vlp 는 매번 재번역하는데, 수정 재렌더에서 **고치지 않은 줄**이 비결정적으로 "
        "흔들려 검수자가 본 문구와 다른 본이 렌더된다(편집실 계약 '고치면 그 항목만'). "
        "원문이 하나라도 다르면 전량 재번역 — 좌표(entries 순번)가 달라지기 때문.",
    "dub.dub_from_video":
