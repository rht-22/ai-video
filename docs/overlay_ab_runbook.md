...
### ✅ 실측 (2026-08-25 · mm-06 · `5b2NhVS2h_o` route BC)

```
탐지        샘플프레임 23 · 영역 105 · 검수필요 77   양쪽 동일
인페인팅    lama 337/337                            구 16:44 · 신 17:10
clean       "텍스트 재렌더·번역 생략"                양쪽 · render {} · 백체크 줄 없음
QA          플래그 334                              양쪽 동일
원문(OCR)   탐지 105영역 동일                        (detections.json 으로 잰다)
길이        11.245s = 11.245s
라우드니스   -20.10 = -20.10 LUFS
판정        ✅ 회귀 0
```

구 엔진도 `--level BC` 를 받았다(거부 걱정은 기우였다 — 거부하는 것은 **더빙 단계**다).

⚠ **첫 판정은 절반이 거짓이었다.** BC 는 번역·자막 이벤트를 아예 안 만드는데 도구가
빈 목록끼리 비교하고 `0항목 동일`·`최대 오차 0.0s` 로 읽어 ✅ 를 냈다 — 실제로 잰 것은
길이·라우드니스뿐이었다. `overlay_ab` 를 고쳐(비교 0개는 판정 제외 · 전부 비면 실패 ·
번역 없는 route 는 detections.json 으로 OCR 축을 잰다) 다시 돌린 것이 위 결과다.
