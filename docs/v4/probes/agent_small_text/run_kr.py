...
PROMPT = """이 영상의 각 화면에는 한국어 문구가
세로로 2줄 적혀 있다. 위쪽 줄일수록 글자가 작다. 화면은 2초마다 바뀌고, 바뀔 때마다
문구도 전부 새로 바뀐다. 화면은 총 8개다.

화면에 실제로 보이는 문구를 전부 그대로 읽어라.
- 화면이 나오는 순서대로, 각 화면 안에서는 위에서 아래로.
- 같은 화면이 여러 프레임에 걸쳐 나오면 그 화면은 한 번만 적어라.
- 글자가 너무 작아서 못 읽은 줄은 빈 문자열 ""로 두어라. 절대 지어내지 마라.
- 맞춤법을 고치지 말고 화면에 있는 글자 그대로. 띄어쓰기도 그대로.
  추측해서 채우지 마라. 확신이 없으면 "" 로 두는 편이 낫다.
- 각 화면은 반드시 2개 항목(못 읽은 줄은 "")으로 답하라 — 줄 순서가 중요하다.

JSON 으로만 답하라:
{"screens": [["푸른 바람 열둘", "", "낡은 골목 스물", "", "", "붉은 손끝 서른"], ...]}"""
