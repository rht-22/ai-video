...
비용
v3의 58%
후보 12개를 뽑고 화면까지 보면서도 편당 ≈75만 토큰. 현행 v3는 ≈129만.

fps 규칙
min(파일fps, 24, 컨텍스트)
시간은 병목이 아니다 — 어떤 길이든 지연이 1분 근방으로 수렴한다. 60분이면 fps 3까지.
...
| 제안 | 판정 | 근거 한 줄 |
|---|---|---|
| ① proxy fps 1 → 2 | 채택 (실측 완료) | 60분 56.5만 토큰으로 성립. **fps 요청이 실제로 더 많은 프레임을 준다는 것이 실측으로 확정됐다** — §2-D. |
| ① media_resolution 기본값 사용 | 채택 (무해) | **영상에서 기본값 = LOW = MEDIUM** — 토큰도 정확도도 동일함을 3회 독립 확인. 명시하든 안 하든 같으므로 운영자 지시대로 **기본값(미지정)**으로 간다. 단 구글이 기본값을 바꾸면 조용히 따라 움직이므로, run_log에 그 실행의 **실측 프레임당 토큰**을 남겨 변화가 드러나게 한다. |
...
> **실험 ② 431초(7.2분) · 희소 신호** — 장편에서도 같다. 대부분 검은 화면이고 20곳에서 400ms만 숫자가 번쩍인다.
...
### offset 판정은 실렌더와 순위가 같다 — 다만 "튐"을 조금 더 세게 본다
...
> offset 멀티파트로 간다. 단 **순위 용도로만** 쓰고 절대 임계에는 쓰지 않는다(§4 임계 항목과 한 몸).
...
> **지연은 fps가 아니라 입력 토큰에 선형이고 기울기가 완만하다** — `t ≈ 8.05초 + 75.7초 × (토큰/100만)` (R²=0.973, 실측 54.7k~852k 토큰). fps 1에서 컨텍스트 상한까지 올리는 대가가 30분 소재 +45초 · 60분 소재 +36초뿐이다. **시간 예산을 따로 둘 필요가 없다.**
...
| 60분 (표준) | 3.4 | 3 | 802,800 | ≈69초 | 제안한 fps 2보다 올릴 여지가 있다 |
| 90분 | 2.1 | 2 | 847,800 | ≈72초 |  |
| 120분 | 1.5 | 1 | 655,200 | ≈58초 | fps로 못 푼다 — **분할이 유일한 길** |

**지연 열이 평평한 것을 보라**(58~72초). 올릴 수 있는 만큼 올려도 시간은 늘 1분 근방으로 수렴한다 — 소재가 길수록 상한 fps가 낮아져 토큰이 비슷한 지점에 머물기 때문이다. **그래서 "시간 대비"를 고민할 필요 없이 컨텍스트 85%를 목표로 역산해 꽂으면 된다.**
...
| 훅 강도 (첫 2초 안에 사건이 있는가) | 화면을 봐야 안다 | ✅ LLM |
...
flowchart TD
  ...
  E --> F["6 후보 편성 — LLM 1콜 (fps2·기본)후보 12~16개 · 각 id·조각목록·한줄사유"]
  F --> G["7 결정적 깔때기 — LLM 0콜 · 무료12~16 → 8"]
  G --> H["8 시각 사고 플래그 — LLM 8콜offset 멀티파트 · 이진 플래그만"]
  H --> I["9 순위 확정 — 코드8 → top-3 · 전량 나빠도 1위는 낸다"]
  I --> J["10 살 붙이기 — LLM 1콜 (승자만)제목·TTS·설명문구·효과문구·유튜브설명"]
  J --> K["11 편집재료 → 렌더 720p → 스타일→ 재렌더 1080p → 검증"]
...
| 8 | 시각 사고 플래그 (offset·60s·fps5·기본) | 8 | LLM 관찰 + 코드 환산 | 8 | 170,400 |
| 9 | 순위 확정 | → top-3 | 코드 | 0 | 0 |
| 10 | 살 붙이기 (승자만) | 3 | LLM | 1 | ≈15,000 |
|  | **합계** |  |  | **10** | **≈751,000** |

**대조**: v3 현행 편당 입력 = Stage1 365,820 + 청크 896,460 + style 25,260 = **1,287,540**. 이 설계는 **v3의 58%**다 — 후보를 12개 뽑고 화면까지 보면서도 지금보다 싸다.
...
3. **후보 수 상향 + 결정적 깔때기만** (LLM 판정 0콜) — 골든 채점과 exception 7편으로 회귀 0 확인.
4. **시각 사고 플래그** (offset·이진·환산은 코드).
5. **쌍대 토너먼트** — 3·4로도 순위가 안 갈릴 때만. 열기 전에 `story.py:488` 주석을 고치는 커밋을 동반할 것(**원칙을 몰래 깨지 않는다**). 상위 4개 녹아웃 + AB/BA 스왑 = 6콜·+25.6만 토큰.
6. **임계 게이트** — 마지막. 그것도 `and 통과분 ≥ 1` 조건과 함께.
...
### Q1. 후보를 여러 개 뽑는 목적은 "한 편을 더 잘 뽑기"인가 "여러 편 내기"인가?
> best-of-N(1편 선택)이면 top-1 최고점 동점 99%가 그대로 걸려 쌍대가 사실상 필수다. top-3 발행이면 훨씬 관대하다.
> 현행 `max_shorts_count=3` 유지 — 상위 3개를 뽑는 건 1등을 맞히는 것보다 쉽고, 깔때기 마지막 층을 단순하게 유지한다.

### Q2. 후보 판정이 화면 글자(원본 번인 자막)를 읽어야 하는가?
> 아니오면 위 설계 그대로. 예면 프록시가 아니라 **원본을 업로드**해야 하고 fps를 0.77 이하로 낮춰야 하며 비용 4배다.
> 아니오 — 별도 축으로 분리한다. 번인 자막 회피는 이미 E17-2/E18-6이 픽셀로 처리한다.

### Q3. 임계 미달로 그날 그 편이 0개 나가도 되는가?
> 안 된다 — top-K + `candidate_rank_fallback`. 무인 6대 노드에서 결번은 조용한 사고이고 재실행은 편당 비용 2배다.

### Q4. `story.py:488`의 M9 원칙(LLM 심사 금지)을 열 것인가?
> 닫아 둔다 — 8단계는 이진 사고 플래그만, 순위는 코드 루브릭. 여기까지가 지금 근거로 방어되는 최대치다. 열려면 반드시 쌍대 토너먼트로(절대 점수 금지) + 주석 수정 커밋 동반.
...
| **fps 상향이 편집 판단을 개선하는가** | 미검증 — §2-D가 증명한 것은 메커니즘이지 품질이 아니다. 합성 소재의 숫자 회수와 실소재의 후보 편성은 다른 과제다. **이제 이것이 가장 중요한 미결** — §8-3 A/B의 몫 |
| offset 파트 경계의 '이음새 판정' 등가성 | 직접 실측 확정 — 실소재 후보 6개·2라운드. 순위 완전 일치, 개별 판정 88%. offset의 "튐 과다 판정" 편향은 순서를 안 바꾼다(§2-E) |
...
| 지연 = 8.05 + 75.7×(토큰/100만) | 회귀·R²0.973 — 실측 54.7k~852k 토큰 구간. ⚠ 출력이 짧은 조건(300~400토큰)이라 **실제 v4처럼 긴 산출에서는 다를 수 있다**. 같은 조건 지연이 최대 2.4배 흔들림 |
