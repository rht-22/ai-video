...
{{"narrations": [
  ...
  {{"before_beat": 3, "text": "…", "cover": ["sp0140"], "closed": false, "hold": true}},
...
def beats_block(beats: list[dict], span_index: dict[str, dict],
    ...
    for i, b in enumerate(beats):
        ...
        out.append(f"\n[{i}] {b['role']} · 씬 m{b['scene']:03d} {r.get('content', '')}"
                   ...
                   + (" · ⚠ 무대사 구간 — 이 화면들 위에 얹을 내레이션 필수(before_beat: "
                      f"{i}, cover 는 이 비트의 화면 조각)" if b.get("silent") else ""))
...
def validate_narrations(resp: Any, n_beats: int, *,
    ...
    for k, n in enumerate(resp.get("narrations") or []):
        ...
        g["hold"] = bool(g.get("hold")) or n.get("hold") is True
        for x in cover:
