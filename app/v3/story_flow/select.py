...
# 2026-09-10 사용자 지시("제목이 너무 길어서 눈에 안 들어와 — 짧게"): 16 → 12(공백 포함). 수작업 v9 「이사 인사
# 갔다가 / 앞집한테 개무시당함」(8·10자) · 벤치마크 2,712편 실측(줄당 9~11자·명사구 종결 95%)에 맞춘다. 짧을수록
# 렌더가 글자를 키운다(fit_title_sizes). legacy `story.TITLE_MAX_CHARS`(16)는 프롬프트 동결이라 그대로.
TITLE_MAX_CHARS = 12          # 상단 밴드 2줄 각각의 상한(공백 포함) — human 흐름
...
- 비트 역할: hook(관심을 끄는 장면 — **재미·시선이 최우선**, 인사·자기소개·상황 설명 대사 금지 · **첫 3초가 정지면 안 된다** — 긴 무성 조각으로 열지 말고 발화·컷이 바로 오는 조각에서 시작) · build · turn · climax(핵심 대사는 통째로) · reaction · ending(펀치·선언·떡밥 대사 직후 뚝 — 해소·정리 장면 금지) · **hook_return**(훅 장면을 원본 순서상 제자리에서 한 번 더 — 선택).
