...
class DesignConfig:
    ...
    # E10: 영상 밴드의 가로 크기(캔버스 px, 320~1080). None = **미지정**: 렌더는 꽉 찬
    # 폭(1080)이고 자막·TTS margin 도 종전 기하(세로 중앙 가정·절대 tts_line_y_margin)
    # 그대로다 — 사람이 실렌더로 픽셀 튜닝한 기존 채널(한 입 주막 video_y=440 등)의
    # 출력이 조용히 움직이면 안 된다(8/21 발주 검수 교정). **명시하면**(1080 포함)
    # 자막·TTS 가 밴드 하단 앵커로 전환된다 — 오케스트레이터 editor_e10 게이트 뒤에서만
    # 들어오므로 '명시 = 신규 채널'이 곧 회귀 0 조건이다. 화면비(aspect_ratio)는 밴드의
    # **모양**을, video_width 는 **크기**를 정한다(scaled_h = int(video_width*r_h/r_w)).
    # 가로는 항상 중앙 배치.
    # ⚠ E10 이전 기본값은 800 이었지만 렌더러가 이 필드를 읽지 않아 무의미했다 —
    # 읽기 시작하면서 None 으로 바꿨다(800 이 살아나면 기존 채널 전부 800px 로 줄어든다).
