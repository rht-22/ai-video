...
ALLOWED_MODELS = {"gemini-3.7-flash", "gemini-3.8-flash"}     # 레포 모델 규칙(CLAUDE.md) — 그 외는 금지 모델
...
class Gemini:
    def __init__(self, api_key: str | None = None, *, log=print):
        ...
        self.types = types
        self.video_model = resolve_model(os.getenv("GEMINI_MODEL_NAME"), log=log)
        self.text_model = resolve_model(os.getenv("GEMINI_FLASH_MODEL_NAME"), log=log)
