...
- `transcript`: 단 한 명의 주요 발화. 내레이션/독백/VO면 `[내레이션]` 접두.
...
| 순위 | 필드 | 성격 | 사용처 |
|------|------|------|--------|
| 1 | `event_template` | 행동·관계·화자를 구조화한 라벨 | 화자·행동·관계 단정의 1순위 근거 |
| 2 | `transcript` | Gemini가 받아쓴 핵심 발화 (다화자 혼재 가능) | 실제 대사 내용 확인용. 화자 단정 불가 |
| 3 | `description` | LLM의 시각 분석 추정 | 장면 묘사. 통화 상대·등장 인물 등 음성 의존 정보는 부정확 가능 |
| 4 | 너의 해석 | 위 셋에 근거 없으면 출력 금지 | — |

**title·라벨 작성 규칙:**
- event_template과 transcript·description이 충돌하면 → event_template을 따른다
- transcript와 description이 충돌하면 → transcript를 따른다 (단, 화자 단정에는 사용 불가)
...
스키마 주의:
- `thread` 필드를 storyline마다 먼저 작성. 클립 선택의 출발점.
- storytelling은 `clip_selection_plan`을 storyline 본체보다 *먼저* 작성. plan을 따라 본체를 옮겨 적는다.
- build/payoff는 `linkage_signals` → `linkage_in` → `chunk_index`/`candidate_index` 순서. **linkage_signals 길이 ≥ 2 강제**.
- `description`, `start_sec`, `end_sec`은 후처리에서 candidate lookup으로 덮어쓰여진다(_resolve_clip_times). 그래도 스키마에는 출력해야 한다 (포맷 일관성). LLM의 시간/description 출력값이 무시되는 것 뿐, 필드는 비우지 말 것.
- tts_line 출력 금지 (별도 단계에서 결정).
...
```
topic: "타인의 비밀인 붉은 선을 보는 소녀" (thread: 능력 발견·사용)
hook:   현흡이 창밖 선아의 머리 위 붉은 선들을 본다  ✓ (능력 도입)
build:  현흡이 현관문 살짝 열고 사과 봉투를 낚아챈다  ✗
        - linkage_signals = [a] 단 1개 (같은 인물만)
        - 능력 thread와 무관한 일상 행동 → (c)/(d)/(e) 모두 부재
        - 시청자는 "S라인 보는 소녀 → 사과 훔치는 소녀"로 다른 영상 인식
payoff: 현흡이 헬멧남 신고가 잘못이었다고 고백
```

올바른 build 선택지:
- 현흡이 *또 다른 인물의 S라인을 본다* → 능력 thread 연속, 신호 (a)+(c)+(f)
- 또는 현흡이 그 능력으로 *직접 신고로 이어진 관찰*을 한다 → payoff와 (c)+(d)+(e) 연결

핵심: build를 고를 때 hook과 payoff *양쪽 모두*와 신호 ≥ 2개씩 묶이는지 확인. 한쪽과만 이어지면 thread가 끊긴다.

위 케이스를 clip_selection_plan으로 작성하려 하면, build의 linkage_to_prev에 "신현흡(a)만"이라고 적게 되는 순간 모델 스스로 "신호 2개 못 채움 → 다른 candidate를 찾아야 함"을 인지하게 된다. 
사후 체크리스트였다면 본체를 다 쓴 뒤에야 발견했을 문제가 plan 작성 중에 막힌다 — 이게 plan을 본체보다 먼저 쓰게 하는 이유다.
"""
...
class GeminiConfig:
    ...
    max_retries: int = 3
    analysis_temperature: float = 1.0               # analyze_chunk / analyze_video_intent
    relationship_temperature: float = 1.0           # 관계 추출 (RELATIONSHIP_EXTRACTION_PROMPT)
    story_temperature: float = 1.0                  # compose_story_with_context
    tts_cues_temperature: float = 1.0               # TTS cues 생성
    shorten_temperature: float = 1.0                # 나레이션 텍스트 축약
    research_temperature: float = 1.0               # work_researcher (Google Search grounding)
    analysis_thinking_level: str = "high"           # "minimal" | "low" | "medium" | "high"
    relationship_thinking_level: str = "high"       # "minimal" | "low" | "medium" | "high"
    story_thinking_level: str = "high"              # "minimal" | "low" | "medium" | "high"
    tts_cues_thinking_level: str = "high"           # "minimal" | "low" | "medium" | "high"
    shorten_thinking_level: str = "high"            # "minimal" | "low" | "medium" | "high"
    research_thinking_level: str = "high"           # "minimal" | "low" | "medium" | "high"
        ...
        for attempt in range(self.config.max_retries):
            try:
                response = self.client.models.generate_content(
                    ...
                    config=self.types.GenerateContentConfig(
                        response_mime_type="application/json",
                        temperature=self.config.tts_cues_temperature,
                        thinking_config=self.types.ThinkingConfig(
        ...
        for attempt in range(2):
            try:
                response = self.client.models.generate_content(
                    ...
                    contents=[prompt],
                    config=self.types.GenerateContentConfig(
                        temperature=self.config.shorten_temperature,
                        thinking_config=self.types.ThinkingConfig(
...
def load_gemini_client() -> GeminiClient:
    ...
    analysis_temperature = float(os.getenv("GEMINI_ANALYSIS_TEMPERATURE", "1.0"))
    relationship_temperature = float(os.getenv("GEMINI_RELATIONSHIP_TEMPERATURE", "1.0"))
    story_temperature = float(os.getenv("GEMINI_STORY_TEMPERATURE", "1.0"))
    tts_cues_temperature = float(os.getenv("GEMINI_TTS_CUES_TEMPERATURE", "1.0"))
    shorten_temperature = float(os.getenv("GEMINI_SHORTEN_TEMPERATURE", "1.0"))
    research_temperature = float(os.getenv("GEMINI_RESEARCH_TEMPERATURE", "1.0"))
    analysis_thinking_level = os.getenv("GEMINI_ANALYSIS_THINKING_LEVEL", "high")
    relationship_thinking_level = os.getenv("GEMINI_RELATIONSHIP_THINKING_LEVEL", "high")
    story_thinking_level = os.getenv("GEMINI_STORY_THINKING_LEVEL", "high")
    tts_cues_thinking_level = os.getenv("GEMINI_TTS_CUES_THINKING_LEVEL", "high")
    shorten_thinking_level = os.getenv("GEMINI_SHORTEN_THINKING_LEVEL", "high")
    research_thinking_level = os.getenv("GEMINI_RESEARCH_THINKING_LEVEL", "high")
    return GeminiClient(GeminiConfig(
        ...
        tts_cues_temperature=tts_cues_temperature,
        shorten_temperature=shorten_temperature,
